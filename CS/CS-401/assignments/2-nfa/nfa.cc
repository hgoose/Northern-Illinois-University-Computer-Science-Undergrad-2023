...
bool proc_string(const string& str, const vector<short>& accepting, transition_table& transitions);
...
int main(int argc, const char* argv[]) {
    ...
    for (const auto& item : strings) {
        outcomes.push_back(proc_string(item, accepting, transitions));
    }
...
bool proc_string(const string& str, const vector<short>& accepting, transition_table& transitions) {
    ...
    for (const auto& character : str) {
        symbol = character - 'a'; // Get the index
...
// Function to print the table and all the outcomes
void print_table(short& nstates, vector<short>& accepting, transition_table& transitions) {
    cout << right << setw(6)  << "| " << "a " << "b" << endl
        << "----+----" << endl;
