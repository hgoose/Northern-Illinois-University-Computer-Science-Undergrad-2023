...
\begin{document}
    ...
    \begin{cppcode}
        ...
        */
        ...
        \bigbreak \noindent 
        \nt{Does not return on success because the calling process's image is entirely replaced}
	...
	if (rs == 0) { // child process	1
        ...
        \end{cppcode}
        \bigbreak \noindent 
        \nt{scanf can lead to issues if the input doesn't match the expected format. It's often better to read a whole line of input and then parse it.}
        \bigbreak \noindent 
        \subsection{getchar}
        \bigbreak \noindent 
        ...
        \end{cppcode}
        \bigbreak \noindent 
        \nt{fgets includes the newline character (\textbackslash n) in the buffer if there's space, so you might want to remove it after capturing input.}
