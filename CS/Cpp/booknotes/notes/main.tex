...
std::cin.get(ch)
    ...
    \pagebreak 
    \subsection{Ingnore the rest of the line}
    \bigbreak \noindent 
    For this we use
    \bigbreak \noindent 
    \begin{cppcode}
    ...
    // Say we read some data, into a and b, where both these values are on the same line in the input file.
    input_file >> a >> b;

    // Then we want to just start grabbing lines, but we need to skip the rest of the first line. We do
    ...
    \bigbreak \noindent 
    \texttt{std::numeric\_limits<T>} is a C++ template that provides information about the properties of a given data type $T$. It allows you to query things like the maximum, minimum, and precision of the type.
    \bigbreak \noindent 
    \textbf{Example:} \texttt{std::numeric\_limits<int>::max()} returns the largest possible value an int can hold.


    \subsubsection{streamsize}
    \bigbreak \noindent 
    \texttt{std::streamsize} is a type used in C++ for representing the size of input/output operations (like how many characters are read or written). It is typically a typedef for a large integer type (e.g., long long).
    \bigbreak \noindent 
    \subsubsection{max}
    \bigbreak \noindent 
    \texttt{max()} is a member function of \texttt{std::numeric\_limits<T>} that returns the maximum finite value that a type $T$ can represent.

    \pagebreak 
    \unsect{Redirecting file to program (./program < file)}
    \bigbreak \noindent 
    ...
    // If file data.txt has 
    10
    20
    30

    Then ./program < data.txt has output 
    10
    20
    30
    \end{cppcode}
    ...
    \bigbreak \noindent 
    \subsection{Passing cin to a function}
    \bigbreak \noindent 
