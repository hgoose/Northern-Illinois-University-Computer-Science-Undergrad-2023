...
\begin{document}
    ...
    \begin{itemize}
        ...
        \item \textbf{git add --patch}: Lets you stage changes \textbf{piece by piece} instead of staging an entire file.
            \bigbreak \noindent 
            \begin{bashcode}
                git add --patch 
            \end{bashcode}
        ...
        \item \textbf{Staging area manipulations}: Staging is represented by a directory \textbf{index} instead .git. The commands that 
            \begin{itemize}
                ...
                \item \textbf{git reset}: git reset can move HEAD, reset the index, and sometimes reset the working tree depending on the mode.
                    ...
                    \begin{enumerate}
                        \item Move the current branch/HEAD back to HEAD$\sim$1.
                        \item Reset the index to match the tree of HEAD$\sim$1.
                        \item Leave the working tree alone.
                    \end{enumerate}
                    \bigbreak \noindent 
                    \begin{bashcode}
                        git reset --hard HEAD
        ...
        \item \textbf{Blobs}: a blob is the object type that stores the contents of a file.
            ...
            \end{itemize}
        \item \textbf{git gc and git prune}: 
            \bigbreak \noindent 
            \begin{bashcode}
                git gc
            \end{bashcode}
            \bigbreak \noindent 
            Means \textbf{Git garbage collection}. It is a maintenance command that cleans and optimizes the internal .git database. It does several things, including:
            \begin{enumerate}
                \item \textbf{Compresses loose objects into packfiles}: When Git creates objects, such as blobs, trees, commits, and tags, they may initially be stored as individual files under:
                    \bigbreak \noindent 
                    \begin{bashcode}
                        .git/objects/
                    \end{bashcode}
                ...
                \item \textbf{Removes unreachable objects, but conservatively}: Objects that are not reachable from any branch, tag, stash, reflog, or other reference may eventually be deleted.
                    \bigbreak \noindent 
                    \begin{bashcode}
                        git add file.txt
                        git reset file.txt
                ...
                \item \textbf{Cleans up other internal metadata}: It may also prune old reflog entries, repack objects, clean redundant packfiles, and optimize internal repository storage.
            \end{enumerate}
            \texttt{git prune} is more specific. It removes \textbf{unreachable loose objects} from the object database 
            \bigbreak \noindent 
            \begin{bashcode}
                git prune
            \end{bashcode}
        ...
        \item \textbf{git show}: used to view detailed metadata and textual differences (diffs) for specific Git objects, most commonly commits. By default, running it without any arguments displays the author, date, log message, and code changes of the absolute latest commit (HEAD) in your current branch
            \bigbreak \noindent 
            \begin{bashcode}
            git show
            \end{bashcode}
        ...
        \item \textbf{Inspection differences}: 
            \begin{center}
                \begin{tabularx}{\textwidth}{@{}lXX@{}}
                    \toprule
                    \textbf{Command}& \textbf{Primary Scope} &\textbf{Default Focus} \\
                    \midrule
                    git show &A single object/commit. &Displays complete metadata and the full code diff.\\[2ex]
                    git log &A chronological list or range of commits. &Displays only structural metadata like hashes, authors, and log messages.\\[2ex]
    ...
    \begin{itemize}
        \item \textbf{Intro}: 
            ...
            \bigbreak \noindent 
            The real difference is that in the Subversion case, your “list of next actions” is always deter- mined by looking at the current working tree. In Git, the “list of next actions” is the contents of the index, which represents what will become the next state of HEAD, and that you can manipu- late directly before executing commit. This gives you an extra layer of control over what’s going to happen, by allowing you to stage those changes in advance.
    \end{itemize}

    \pagebreak 
    \subsection{Reset}
    \begin{itemize}
        \item \textbf{Doing a mixed reset}: If you use the --mixed option (or no option at all, as this is the default), reset will revert parts of your index along with your HEAD reference to match the given commit. The main difference from --soft is that --soft only changes the meaning of HEAD and doesn’t touch the index.
            \bigbreak \noindent 
            \begin{bashcode}
            ...
            \end{bashcode}
        \item \textbf{Doing a soft reset}: If you use the --soft option to reset, this is the same as simply changing your HEAD reference to a different commit. Your working tree changes are left untouched. This means the following two commands are equivalent:
            \bigbreak \noindent 
            \begin{bashcode}
                git reset --soft HEAD^
        ...
        \item \textbf{Doing a hard reset}: A hard reset (the --hard option) has the potential of being very dangerous, as it’s able to do two different things at once: First, if you do a hard reset against your current HEAD, it will erase all changes in your working tree, so that your current files match the contents of HEAD
            \bigbreak \noindent 
            There is also another command, \texttt{checkout}, which operates just like reset --hard if the in- dex is empty. Otherwise, it forces your working tree to match the index.
            \bigbreak \noindent 
            ...
            \begin{enumerate}
                \item It saves your work in the stash, which you can come back to at any time. Note that the stash is not branch specific, so you could potentially stash the state of your tree while on one branch, and later apply the differences to another.
                \item It reverts your working tree back to a past state, but on a new branch, so if you decide to commit your changes against the past state, you won’t have altered your original branch
            \end{enumerate}
            ...
            \begin{bashcode}
                ...
                git branch --move new-branch master # the new-branch is now my master
            \end{bashcode}
            \bigbreak \noindent 
            ...
            \end{bashcode}
            \bigbreak \noindent 
            \textbf{Note:} Consider the situation 
            \bigbreak \noindent 
            \begin{bashcode}
                git reset --hard HEAD~3
            \end{bashcode}
            \bigbreak \noindent 
            while on \texttt{master}. That command does three things:
            \begin{enumerate}
                \item Moves the current branch pointer backward to HEAD~3.
                \item Resets the index to match that older commit.
                \item Resets the working tree to match that older commit, discarding uncommitted changes.
            \end{enumerate}
            ...
            \begin{cppcode}
                ...
                D -- E    no longer referenced by master
            \end{cppcode}
            \bigbreak \noindent 
    ...
    \begin{itemize}
        ...
        \item \textbf{The reflog}:  The first of these is the Git reflog, a kind of meta-repository that records — in the form of commits — every change you make to your repository. This means that when you create a tree from your index and store it under a commit (all of which is done by commit), you are also inadvertently adding that commit to the reflog, which can be viewed using the following command:
            \bigbreak \noindent 
            \begin{bashcode}
            git reflog
        ...
        \item \textbf{Stash}: The other place blobs can exist, albeit indirectly, is in your working tree itself. What I mean is, say you’ve changed a file foo.c but you haven’t added those changes to the index yet. Git may not have created a blob for you, but those changes do exist, meaning the content exists — it just lives in your filesystem instead of Git’s repository. The file even has its own SHA1 hash id, de- spite the fact no real blob exists. You can view it with this command:
            \bigbreak \noindent 
            \begin{bashcode}
            ...
            \bigbreak \noindent 
            \begin{bashcode}
            git stash clear # BAD, loose all history
    ...
    \pagebreak 
    \unsect{Further insight}
    \subsection{Git rm}
    \begin{itemize}
        \item \textbf{Remove tracked files from remote}: Suppose we have a directory
            \bigbreak \noindent 
            \begin{bashcode}
            somedir/
            \end{bashcode}
