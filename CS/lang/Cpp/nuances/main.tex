...
\begin{document}
 ...
 \end{cppcode}
    ...
    \bigbreak \noindent 
    In C++, a const object must have all of its data members fully initialized at construction time
    \bigbreak \noindent 
    In the code above, the default constructor is defaulted, which means it does nothing more than “default‐initialize” each member. For a built‐in type like int, default initialization leaves it with an indeterminate value. When you try to create a const object of type A
    \bigbreak \noindent 
    the member i would remain uninitialized, which is not allowed because a const object must be initialized completely. To make A const default constructible, you need to provide an initializer for i
    \bigbreak \noindent 
    \begin{cppcode}
        // Make const default constructable
        struct A {
            A() { i=0; }
            int i;
        };
    \end{cppcode}
    \bigbreak \noindent 
    We can also simply do the following
    \bigbreak \noindent 
    \begin{cppcode}
        struct A {
            A() = default;
    ...
    \end{cppcode}
    \bigbreak \noindent 
    Note that types like strings are default initialized with a determinant value, the empty string, so the following will not cause an error
    \bigbreak \noindent 
    \begin{cppcode}
        struct A {
            A() = default;
...
\pagebreak 
\unsect{Argument dependent name lookup (ADL/Koenig lookup)}
\bigbreak \noindent 
Consider the code
\bigbreak \noindent 
\begin{cppcode}
...
We get output TS. 
\begin{itemize}
    \item \textbf{First Call (adl(S()) inside call_adl):} This call is non-dependent (its argument is of a fixed type, S, that does not depend on the template parameter). Name lookup for non-dependent calls is performed when the template is defined. At that point, only the template version of adl is visible (the non-template adl(S) is defined later). Thus, the call resolves to the template function, which prints "T".
    \item \textbf{Second Call (adl(t)):} Here, t is a template parameter and the call is dependent. Its lookup is deferred until instantiation. At instantiation time, both overloads are considered. Overload resolution then selects the non-template adl(S) because it is a better match (an exact match for type S) compared to the template version. This function prints "S".
\end{itemize}
\bigbreak \noindent 
Dependent calls within a template—like the call to adl(t)—are not fully resolved until the template is instantiated. In this case, instantiation happens when you call call_adl(S()). At that point, the compiler performs name lookup for adl(t) using the actual type of t (which is S), and because it’s a dependent call, it considers all overloads available at instantiation, selecting the non-template adl(S).
\begin{itemize}
    \item \textbf{Non-dependent call (adl(S())):} Looked up at definition time.
    \item \textbf{Dependent call (adl(t)):} Instantiated and looked up during the template instantiation phase when call_adl is called with a specific type (here, S).
\end{itemize}
...
\unsect{Notes about pointers}
    ...
    \pagebreak 
    \unsect{Notes about function types}
    \bigbreak \noindent 
    ...
    \pagebreak 
    \unsect{Notes about name resolution}
    \bigbreak \noindent 
    \begin{cppcode}
        int foo() {
            return 10;
    ...
    \pagebreak 
    \unsect{Notes about default parameters}
    \bigbreak \noindent 
    ...
    \pagebreak 
    \unsect{Notes about the this pointer}
    \bigbreak \noindent 
    ...
    \pagebreak 
    \unsect{Argument evaluation order: Indeterminately sequence}
    \bigbreak \noindent 
    Consider the code
    \bigbreak \noindent 
    \begin{cppcode}
    ...
    \bigbreak \noindent 
     ...
     \pagebreak 
     \unsect{Do static members increase the size of a class or object?}
     \bigbreak \noindent 
     ...
     \begin{cppcode}
         ...
         template<template<typename T = B> typename C>
         void g() {
             C<>::f();
         }
     \end{cppcode}
     ...
     \end{cppcode}
     \bigbreak \noindent 
     means that the function template g is parameterized by a class template C. Here, C itself is a template that takes one type parameter T with a default type of B. This implies that C must be a template that can be instantiated with a single type argument, and if none is provided, it will use B by default.

     \pagebreak 
     \unsect{Pointers vs references}
     \bigbreak \noindent 
     \begin{itemize}
         \item Once a reference is created, it cannot be later changed to reference another object; it cannot be reseated. On the other hand, this is often done with pointers.
         \item References cannot be NULL/nullptr. Pointers are often pointing at the nullptr to indicate that they are not pointing to any valid thing. Though, it’s worth noting that since C++17 there is also std::optional in the standard library if we need to indicate that an object is “not there”.
         \item A reference must be initialized when declared. There is no such restriction with pointers.
     \end{itemize}
     \bigbreak \noindent 
     ...
     \begin{itemize}
         \item \textbf{Safer:} Since references must be initialized, wild references like wild pointers are unlikely to exist. It is still possible to have references that don’t refer to a valid location, they are called dangling references.
         \item \textbf{Easier to use:} References don’t need the dereferencing operator (*) to access the value. They can be used like normal variables. The address of (\&) operator is needed only at the time of declaration. Also, members of an object reference can be accessed with dot operator (.), unlike pointers where arrow operator (->) is needed to access members.
     \end{itemize}

     \bigbreak \noindent 
     \subsection{Wild pointers and dangling references}
     \bigbreak \noindent 
     Wild pointers in C++ are pointers that have not been properly initialized to point to a known, valid memory location. Because they contain garbage values (i.e., unpredictable addresses), dereferencing them can lead to undefined behavior, crashes, or memory corruption.
     \bigbreak \noindent 
     \begin{cppcode}
     int* ptr; // wild
     \end{cppcode}
     \bigbreak \noindent 
     Dangling references occur when a reference in C++ continues to refer to an object that has been destroyed or gone out of scope. Since the reference is still being used after its bound object no longer exists, any access to it results in undefined behavior.
     \bigbreak \noindent 
     \begin{cppcode}
         int* p = new int(5);
         int& ref = *p;
     ...
     \end{cppcode}
     \bigbreak \noindent 
     Outputs 'a'... The std::exit function calls the destructors of all objects with static storage duration, but is not responsible for destroying local variables. Therefore, when std::exit(0); is called, only the destructor of a is called; b is a local variable and its destructor is thus not called.

     \pagebreak 
     \unsect{Immediate destruction}
     \bigbreak \noindent 
     \begin{cppcode}
         class C {
             public:
     ...
     \end{cppcode}
     \bigbreak \noindent 
     We get ouput 127386, since the last two objects are not bound and therefore immediately destructed.
