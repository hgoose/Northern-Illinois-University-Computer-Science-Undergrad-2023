...
\bigbreak \noindent 
\subsection{What is RVO}
\bigbreak \noindent 
RVO stands for return value optimization. It is a compiler optimization that involves eliminating the temporary object created to hold a function’s return value. RVO is
particularly notable for being allowed to change the observable
...
observable behaviour as if (i.e. pretending) all the requirements
of the standard have been fulfilled. This is commonly referred to
as the “as-if rule”. The term return value optimization refers to a
special clause in the C++ standard that goes even further than the
...
function (that will be returned),it realizes it doesn’t need both
variables. Under the covers, the compiler passes the address of the
calling function’s variable to the called function.

\bigbreak \noindent 
\subsection{How can we ensure the compiler performs RVO?}
\bigbreak \noindent 
In order to have RVO applied the returned object has to be constructed on a return statement. A function can have multiple exits,
...
\bigbreak \noindent 
\subsection{What is NRVO and when can it happen?}
\bigbreak \noindent 
N in NRVO stands from Named, so we speak about Named Return
Value Optimization. Copies of temporary objects can be spared
even if the returned object has a name and is therefore not constructed on the return statement. The requirement for an NRVO is
