...
\pagebreak 
\unsect{The Curiously Recurring Template Pattern (CRTP)}
\bigbreak \noindent 
...
class by static\_casting itself into the derived class.
\bigbreak \noindent 
\begin{cppcode}
    template <typename T>
    class Base {
...
use dynamic\_cast here. A dynamic\_cast is used when you want to
...
\begin{cppcode}
    ...
    int main() {
        Derived d;
        // This will call Derived::implementation() through the CRTP interface.
        d.interface();
        return 0;
    }
\end{cppcode}
...
\end{cppcode}
\bigbreak \noindent 
std::basic\_string\_view is a class template that provides a non-owning view into a sequence of characters, whereas std::string\_view is simply a type alias (a specialization) of std::basic\_string\_view for the common case of char types.
\bigbreak \noindent 
\begin{cppcode}
...
\begin{itemize}
    ...
    \item \textbf{c++20}: 
        \begin{itemize}
            ...
            \item string and string\_view methods starts\_with, ends\_with
        \end{itemize}
    \item \textbf{c++23}: 
        \begin{itemize}
