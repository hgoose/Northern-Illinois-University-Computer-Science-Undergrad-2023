...
\begin{document}
    ...
    \subsection{activity (in android manifest)}
        ...
        \begin{itemize}
            \item \textbf{android:completionHint}:	Defines the hint displayed in the drop down menu. 
            \item \textbf{android:completionHintView}:	Defines the hint view displayed in the drop down menu. 
            \item \textbf{android:completionThreshold}:	Defines the number of characters that the user must type before completion suggestions are displayed in a drop down menu. 
            \item \textbf{android:dropDownAnchor}:	View to anchor the auto-complete dropdown to. 
            \item \textbf{android:dropDownHeight}:	Specifies the basic height of the dropdown. 
            \item \textbf{android:dropDownHorizontalOffset}:	Amount of pixels by which the drop down should be offset horizontally. 
            \item \textbf{android:dropDownSelector}:	Selector in a drop down list. 
            \item \textbf{android:dropDownVerticalOffset}:	Amount of pixels by which the drop down should be offset vertically. 
            \item \textbf{android:dropDownWidth}:	Specifies the basic width of the dropdown. 
            \item \textbf{android:popupBackground}:	The background to use for the popup window. 
        \end{itemize}
        ...
        \begin{itemize}
            \item Doesn't have its own XML attributes, inherits from \texttt{ViewGroup} and \texttt{View} 
        \end{itemize}
        ...
        \item \textbf{Public methods (Only most important)}
            \begin{itemize}
                ...
                \item \textbf{void onDraw(Canvas canvas)}: Called to render the view’s visual content.
                    ...
                    % -- Text Stuff -- %
                \item \textbf{public void setTextAlignment (int textAlignment)}:
                \item \textbf{public void setTextDirection (int textDirection)}:
                ...
                \item \textbf{final int	getMeasuredHeightAndState()}: Return the full height measurement information for this view as computed by the most recent call to measure(int, int).
                \item \textbf{final int	getMeasuredState()}: Return only the state bits of getMeasuredWidthAndState() and getMeasuredHeightAndState(), combined into one integer.
                ...
                \item \textbf{final int	getMeasuredWidthAndState()}: Return the full width measurement information for this view as computed by the most recent call to measure(int, int).
                \item \textbf{int	getMinimumHeight()}: Returns the minimum height of the view.
                \item \textbf{int	getMinimumWidth()}: Returns the minimum width of the view.
    ...
    \pagebreak 
    \subsection{view.MeasureSpec}
    \begin{itemize}
        \item \textbf{Hierarchy}
            \begin{center}
                java.lang.Object $\to$	android.view.View.MeasureSpec
            \end{center}
        \item \textbf{Include}
            \bigbreak \noindent 
            \begin{javacode}
        ...
        \item \textbf{Public methods}
            \begin{itemize}
                \item \textbf{static int	getMode(int measureSpec)}: Extracts the mode from the supplied measure specification.
                \item \textbf{static int	getSize(int measureSpec)}: Extracts the size from the supplied measure specification.
                \item \textbf{static int	makeMeasureSpec(int size, int mode)}: Creates a measure specification based on the supplied size and mode.
                \item \textbf{static String	toString(int measureSpec)}: Returns a String representation of the specified measure specification.
            \end{itemize}
        \item \textbf{Constants}
            \begin{itemize}
                \item \textbf{int	AT\_MOST}: Measure specification mode: The child can be as large as it wants up to the specified size.
                \item \textbf{int	EXACTLY}: Measure specification mode: The parent has determined an exact size for the child.
                \item \textbf{int	UNSPECIFIED}: Measure specification mode: The parent has not imposed any constraint on the child.
            \end{itemize}
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Hierarchy}
            \begin{center}
                java.lang.Object $\to$	android.content.res.Resources.Theme
            \end{center}
        \item \textbf{Include}
            \bigbreak \noindent 
            \begin{javacode}
        ...
        \item \textbf{Public methods}
            \begin{itemize}
                \item \textbf{void	applyStyle(int resId, boolean force)}: Place new attribute values into the theme.
                \item \textbf{void	dump(int priority, String tag, String prefix)}: Print contents of this theme out to the log.
                \item \textbf{boolean	equals(Object o)}: Indicates whether some other object is "equal to" this one.
                \item \textbf{int[]	getAttributeResolutionStack(int defStyleAttr, int defStyleRes, int explicitStyleRes)}: Returns the ordered list of resource ID that are considered when resolving attribute values when making an equivalent call to obtainStyledAttributes(android.util.AttributeSet, int[], int, int) .
                \item \textbf{int	getChangingConfigurations()}: Returns a bit mask of configuration changes that will impact this theme (and thus require completely reloading it).
                \item \textbf{Drawable	getDrawable(int id)}: Return a drawable object associated with a particular resource ID and styled for the Theme.
                \item \textbf{int	getExplicitStyle(AttributeSet set)}: Returns the resource ID for the style specified using style="..." in the AttributeSet's backing XML element or Resources.ID\_NULL otherwise if not specified or otherwise not applicable.
                \item \textbf{Resources	getResources()}: Returns the resources to which this theme belongs.
                \item \textbf{int	hashCode()}: Returns a hash code value for the object.
                \item \textbf{TypedArray	obtainStyledAttributes(AttributeSet set, int[] attrs, int defStyleAttr, int defStyleRes)}: Return a TypedArray holding the attribute values in set that are listed in attrs.
                \item \textbf{TypedArray	obtainStyledAttributes(int[] attrs)}: Return a TypedArray holding the values defined by Theme which are listed in attrs.
                \item \textbf{TypedArray	obtainStyledAttributes(int resId, int[] attrs)}: Return a TypedArray holding the values defined by the style resource resid which are listed in attrs.
                \item \textbf{void	rebase()}: Rebases the theme against the parent Resource object's current configuration by re-applying the styles passed to applyStyle(int, boolean).
                ...
                \item \textbf{void	setTo(Resources.Theme other)}: Set this theme to hold the same contents as the theme other.
                \item \textbf{String	toString()}: Returns a string representation of the object.
            \end{itemize}
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Hierarchy}
            \begin{center}
                java.lang.Object $\to $	android.util.TypedValue 
            \end{center}
        \item \textbf{Include}
            \bigbreak \noindent 
            \begin{javacode}
        ...
        \item \textbf{Public methods}
            \begin{itemize}
                \item \textbf{static float	applyDimension(int unit, float value, DisplayMetrics metrics)}: Converts an unpacked complex data value holding a dimension to its final floating point pixel value.
                \item \textbf{static final String	coerceToString(int type, int data)}: Perform type conversion as per coerceToString() on an explicitly supplied type and data.
                \item \textbf{final CharSequence	coerceToString()}: Regardless of the actual type of the value, try to convert it to a string value.
                \item \textbf{static float	complexToDimension(int data, DisplayMetrics metrics)}: Converts a complex data value holding a dimension to its final floating point value.
                \item \textbf{static int	complexToDimensionPixelOffset(int data, DisplayMetrics metrics)}: Converts a complex data value holding a dimension to its final value as an integer pixel offset.
                ...
                \item \textbf{static float	complexToFloat(int complex)}: Retrieve the base value from a complex data integer.
                \item \textbf{static float	complexToFraction(int data, float base, float pbase)}: Converts a complex data value holding a fraction to its final floating point value.
                \item \textbf{static float	convertDimensionToPixels(int unitToConvertFrom, float value, DisplayMetrics metrics)}: Converts a dimension value to raw pixels, e.g. DP to PX.
                \item \textbf{static float	convertPixelsToDimension(int unitToConvertTo, float pixelValue, DisplayMetrics metrics)}: Converts a pixel value to the given dimension, e.g. PX to DP.
                \item \textbf{static float	deriveDimension(int unitToConvertTo, float pixelValue, DisplayMetrics metrics)}: Converts a pixel value to the given dimension, e.g. PX to DP.
                \item \textbf{int	getComplexUnit()}: Return the complex unit type for this value.
                \item \textbf{float	getDimension(DisplayMetrics metrics)}: Return the data for this value as a dimension.
                \item \textbf{final float	getFloat()}: Return the data for this value as a float.
                \item \textbf{float	getFraction(float base, float pbase)}: Return the data for this value as a fraction.
                \item \textbf{boolean	isColorType()}: Determine if a value is a color.
                \item \textbf{void	setTo(TypedValue other)}:
                \item \textbf{String	toString()}: Returns a string representation of the object.
            \end{itemize}
        \item \textbf{Constants}
            \begin{itemize}
                \item \textbf{int	COMPLEX\_MANTISSA\_MASK}: Complex data: mask to extract mantissa information (after shifting by COMPLEX\_MANTISSA\_SHIFT).
                \item \textbf{int	COMPLEX\_MANTISSA\_SHIFT}: Complex data: bit location of mantissa information.
                \item \textbf{int	COMPLEX\_RADIX\_0p23}: Complex data: the mantissa magnitude is 0 bits -- i.e, 0x0.nnnnnn
                \item \textbf{int	COMPLEX\_RADIX\_16p7}: Complex data: the mantissa magnitude is 16 bits -- i.e, 0xnnnn.nn
                \item \textbf{int	COMPLEX\_RADIX\_23p0}: Complex data: the mantissa is an integral number -- i.e., 0xnnnnnn.0
                \item \textbf{int	COMPLEX\_RADIX\_8p15}: Complex data: the mantissa magnitude is 8 bits -- i.e, 0xnn.nnnn
                \item \textbf{int	COMPLEX\_RADIX\_MASK}: Complex data: mask to extract radix information (after shifting by COMPLEX\_RADIX\_SHIFT).
                \item \textbf{int	COMPLEX\_RADIX\_SHIFT}: Complex data: where the radix information is, telling where the decimal place appears in the mantissa.
                \item \textbf{int	COMPLEX\_UNIT\_DIP}: TYPE\_DIMENSION complex unit: Value is Device Independent Pixels.
                \item \textbf{int	COMPLEX\_UNIT\_FRACTION}: TYPE\_FRACTION complex unit: A basic fraction of the overall size.
                \item \textbf{int	COMPLEX\_UNIT\_FRACTION\_PARENT}: TYPE\_FRACTION complex unit: A fraction of the parent size.
                \item \textbf{int	COMPLEX\_UNIT\_IN}: TYPE\_DIMENSION complex unit: Value is in inches.
                \item \textbf{int	COMPLEX\_UNIT\_MASK}: Complex data: mask to extract unit information (after shifting by COMPLEX\_UNIT\_SHIFT).
                \item \textbf{int	COMPLEX\_UNIT\_MM}: TYPE\_DIMENSION complex unit: Value is in millimeters.
                \item \textbf{int	COMPLEX\_UNIT\_PT}: TYPE\_DIMENSION complex unit: Value is in points.
                \item \textbf{int	COMPLEX\_UNIT\_PX}: TYPE\_DIMENSION complex unit: Value is raw pixels.
                \item \textbf{int	COMPLEX\_UNIT\_SHIFT}: Complex data: bit location of unit information.
                \item \textbf{int	COMPLEX\_UNIT\_SP}: TYPE\_DIMENSION complex unit: Value is a scaled pixel.
                \item \textbf{int	DATA\_NULL\_EMPTY}: TYPE\_NULL data indicating the value was explicitly set to null.
                \item \textbf{int	DATA\_NULL\_UNDEFINED}: TYPE\_NULL data indicating the value was not specified.
                \item \textbf{int	DENSITY\_DEFAULT}: If density is equal to this value, then the density should be treated as the system's default density value: DisplayMetrics.DENSITY\_DEFAULT.
                \item \textbf{int	DENSITY\_NONE}: If density is equal to this value, then there is no density associated with the resource and it should not be scaled.
                \item \textbf{int	TYPE\_ATTRIBUTE}: The data field holds an attribute resource identifier (referencing an attribute in the current theme style, not a resource entry).
                \item \textbf{int	TYPE\_DIMENSION}: The data field holds a complex number encoding a dimension value.
                \item \textbf{int	TYPE\_FIRST\_COLOR\_INT}: Identifies the start of integer values that were specified as color constants (starting with '#').
                \item \textbf{int	TYPE\_FIRST\_INT}: Identifies the start of plain integer values.
                \item \textbf{int	TYPE\_FLOAT}: The data field holds an IEEE 754 floating point number.
                \item \textbf{int	TYPE\_FRACTION}: The data field holds a complex number encoding a fraction of a container.
                \item \textbf{int	TYPE\_INT\_BOOLEAN}: data holds 0 to represent false, or a value different from 0 to represent true.
                \item \textbf{int	TYPE\_INT\_COLOR\_ARGB4}: The data field holds a color that was originally specified as #argb.
                \item \textbf{int	TYPE\_INT\_COLOR\_ARGB8}: The data field holds a color that was originally specified as #aarrggbb.
                \item \textbf{int	TYPE\_INT\_COLOR\_RGB4}: The data field holds a color that was originally specified as #rgb.
                \item \textbf{int	TYPE\_INT\_COLOR\_RGB8}: The data field holds a color that was originally specified as #rrggbb.
                \item \textbf{int	TYPE\_INT\_DEC}: The data field holds a number that was originally specified in decimal.
                \item \textbf{int	TYPE\_INT\_HEX}: The data field holds a number that was originally specified in hexadecimal (0xn).
                \item \textbf{int	TYPE\_LAST\_COLOR\_INT}: Identifies the end of integer values that were specified as color constants.
                \item \textbf{int	TYPE\_LAST\_INT}: Identifies the end of plain integer values.
                \item \textbf{int	TYPE\_NULL}: The value contains no data.
                \item \textbf{int	TYPE\_REFERENCE}: The data field holds a resource identifier.
                \item \textbf{int	TYPE\_STRING}: The string field holds string data.
            \end{itemize}
        \item \textbf{Fields}
            \begin{itemize}
                \item \textbf{public int	assetCookie}: Additional information about where the value came from; only set for strings.
                \item \textbf{public int	changingConfigurations}: If the value came from a resource, these are the configurations for which its contents can change.
                \item \textbf{public int	data}: Basic data in the value, interpreted according to type
                \item \textbf{public int	density}: If the Value came from a resource, this holds the corresponding pixel density.
                \item \textbf{public int	resourceId}: If Value came from a resource, this holds the corresponding resource id.
                \item \textbf{public int	sourceResourceId}: If the Value came from a style resource or a layout resource (set in an XML layout), this holds the corresponding style or layout resource id against which the attribute was resolved.
                \item \textbf{public CharSequence	string}: If the value holds a string, this is it.
                \item \textbf{public int	type}: The type held by this value, as defined by the constants here.
            \end{itemize}
    \end{itemize}
    ...
    \pagebreak 
    \subsection{GridLayout.Alignment}
    \begin{itemize}
        \item \textbf{Constants}
            \begin{itemize}
                \item GridLayout.TOP
                \item GridLayout.LEFT
                \item GridLayout.BOTTOM
                \item GridLayout.RIGHT
                \item GridLayout.START
                \item GridLayout.END
                \item GridLayout.CENTER
                \item GridLayout.BASELINE and GridLayout.FILL.
            \end{itemize}
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Hierarchy}
            \begin{center}
                java.lang.Object $\to$	android.view.View $\to$	android.view.ViewGroup $\to$	android.widget.AdapterView<T extends android.widget.Adapter>
            \end{center}
        \item \textbf{Include}
            \bigbreak \noindent 
            \begin{javacode}
        ...
        \item \textbf{Public methods}
            \begin{itemize}
                \item \textbf{void	addView(View child, int index)}: This method is not supported and throws an UnsupportedOperationException when called.
                \item \textbf{void	addView(View child)}: This method is not supported and throws an UnsupportedOperationException when called.
                \item \textbf{void	addView(View child, ViewGroup.LayoutParams params)}: This method is not supported and throws an UnsupportedOperationException when called.
                \item \textbf{void	addView(View child, int index, ViewGroup.LayoutParams params)}: This method is not supported and throws an UnsupportedOperationException when called.
                \item \textbf{CharSequence	getAccessibilityClassName()}: Return the class name of this object to be used for accessibility purposes.
                ...
                \item \textbf{int	getFirstVisiblePosition()}: Returns the position within the adapter's data set for the first item displayed on screen.
                \item \textbf{Object	getItemAtPosition(int position)}: Gets the data associated with the specified position in the list.
                \item \textbf{long	getItemIdAtPosition(int position)}:
                \item \textbf{int	getLastVisiblePosition()}: Returns the position within the adapter's data set for the last item displayed on screen.
                \item \textbf{final AdapterView.OnItemClickListener	getOnItemClickListener()}:
                \item \textbf{final AdapterView.OnItemLongClickListener	getOnItemLongClickListener()}:
                \item \textbf{final AdapterView.OnItemSelectedListener	getOnItemSelectedListener()}:
                \item \textbf{int	getPositionForView(View view)}: Returns the position within the adapter's data set for the view, where view is a an adapter item or a descendant of an adapter item.
                \item \textbf{Object	getSelectedItem()}:
                \item \textbf{long	getSelectedItemId()}:
                \item \textbf{int	getSelectedItemPosition()}: Return the position of the currently selected item within the adapter's data set
                \item \textbf{abstract View	getSelectedView()}:
                \item \textbf{void	onProvideAutofillStructure(ViewStructure structure, int flags)}: Populates a ViewStructure to fullfil an autofill request. It also sets the autofill options in the structure; when overridden, it should set it as well, either explicitly by calling ViewStructure.setAutofillOptions(CharSequence[]) or implicitly by calling super.onProvideAutofillStructure(structure, flags).
                \item \textbf{boolean	performItemClick(View view, int position, long id)}: Call the OnItemClickListener, if it is defined.
                \item \textbf{void	removeAllViews()}: This method is not supported and throws an UnsupportedOperationException when called.
                \item \textbf{void	removeView(View child)}: This method is not supported and throws an UnsupportedOperationException when called.
                \item \textbf{void	removeViewAt(int index)}: This method is not supported and throws an UnsupportedOperationException when called.
                ...
                \item \textbf{void	setFocusable(int focusable)}: Sets whether this view can receive focus.
                \item \textbf{void	setFocusableInTouchMode(boolean focusable)}: Set whether this view can receive focus while in touch mode.
                \item \textbf{void	setOnClickListener(View.OnClickListener l)}: Register a callback to be invoked when this view is clicked.
                \item \textbf{void	setOnItemClickListener(AdapterView.OnItemClickListener listener)}: Register a callback to be invoked when an item in this AdapterView has been clicked.
                \item \textbf{void	setOnItemLongClickListener(AdapterView.OnItemLongClickListener listener)}: Register a callback to be invoked when an item in this AdapterView has been clicked and held
                \item \textbf{void	setOnItemSelectedListener(AdapterView.OnItemSelectedListener listener)}: Register a callback to be invoked when an item in this AdapterView has been selected.
                \item \textbf{abstract void	setSelection(int position)}: Sets the currently selected item.
            \end{itemize}
        \item \textbf{Protected methods}
            \begin{itemize}
                \item \textbf{boolean	canAnimate()}: Indicates whether the view group has the ability to animate its children after the first layout.
                ...
                \item \textbf{void	dispatchSaveInstanceState(SparseArray<Parcelable> container)}: Override to prevent freezing of any views created by the adapter.
                \item \textbf{void	onDetachedFromWindow()}: This is called when the view is detached from a window.
                \item \textbf{void	onLayout(boolean changed, int left, int top, int right, int bottom)}: Called from layout when this view should assign a size and position to each of its children.
            \end{itemize}
        \item \textbf{Constants}
            \begin{itemize}
                \item \textbf{int	INVALID\_POSITION}: Represents an invalid position.
                \item \textbf{long	INVALID\_ROW\_ID}: Represents an empty or invalid row id
                \item \textbf{int	ITEM\_VIEW\_TYPE\_HEADER\_OR\_FOOTER}: The item view type returned by Adapter.getItemViewType(int) when the item is a header or footer.
                \item \textbf{int	ITEM\_VIEW\_TYPE\_IGNORE}: The item view type returned by Adapter.getItemViewType(int) when the adapter does not want the item's view recycled.
            \end{itemize}
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Default methods}
            \begin{itemize}
                \item \textbf{default CharSequence[]	getAutofillOptions()}: Gets a string representation of the adapter data that can help AutofillService autofill the view backed by the adapter.
            \end{itemize}
        \item \textbf{Abstract methods}
            \begin{itemize}
                ...
                \item \textbf{abstract long	getItemId(int position)}: Get the row id associated with the specified position in the list.
                \item \textbf{abstract int	getItemViewType(int position)}: Get the type of View that will be created by getView(int, View, ViewGroup) for the specified item.
                ...
                \item \textbf{abstract int	getViewTypeCount()}: Returns the number of types of Views that will be created by getView(int, View, ViewGroup).
                \item \textbf{abstract boolean	hasStableIds()}: Indicates whether the item ids are stable across changes to the underlying data.
                \item \textbf{abstract boolean	isEmpty()}
                \item \textbf{abstract void	registerDataSetObserver(DataSetObserver observer)}: Register an observer that is called when changes happen to the data used by this adapter.
                \item \textbf{abstract void	unregisterDataSetObserver(DataSetObserver observer)}: Unregister an observer that has previously been registered with this adapter via registerDataSetObserver(DataSetObserver).
            \end{itemize}
        \item \textbf{Constants}
            \begin{itemize}
                \item \textbf{int	IGNORE\_ITEM\_VIEW\_TYPE}: An item view type that causes the AdapterView to ignore the item view.
                \item \textbf{int	NO\_SELECTION}:
            \end{itemize}
    \end{itemize}

    \pagebreak 
    \subsection{BaseAdapter (Abstract)}
    \begin{itemize}
        \item \textbf{Hierarchy} 
            \begin{center}
                java.lang.Object $\to$	android.widget.BaseAdapter
            \end{center}
        \item \textbf{Include}
            \bigbreak \noindent 
            \begin{javacode}
        ...
        \item \textbf{Public methods}
            \begin{itemize}
                \item \textbf{boolean	areAllItemsEnabled()}: Indicates whether all the items in this adapter are enabled.
                \item \textbf{CharSequence[]	getAutofillOptions()}: Gets a string representation of the adapter data that can help AutofillService autofill the view backed by the adapter.
                \item \textbf{View	getDropDownView(int position, View convertView, ViewGroup parent)}: Gets a View that displays in the drop down popup the data at the specified position in the data set.
                \item \textbf{int	getItemViewType(int position)}: Get the type of View that will be created by getView(int, View, ViewGroup) for the specified item.
                \item \textbf{int	getViewTypeCount()}: Returns the number of types of Views that will be created by getView(int, View, ViewGroup).
                \item \textbf{boolean	hasStableIds()}: Indicates whether the item ids are stable across changes to the underlying data.
                \item \textbf{boolean	isEmpty()}:
                \item \textbf{boolean	isEnabled(int position)}: Returns true if the item at the specified position is not a separator.
                \item \textbf{void	notifyDataSetChanged()}: Notifies the attached observers that the underlying data has been changed and any View reflecting the data set should refresh itself.
                \item \textbf{void	notifyDataSetInvalidated()}: Notifies the attached observers that the underlying data is no longer valid or available.
                \item \textbf{void	registerDataSetObserver(DataSetObserver observer)}: Register an observer that is called when changes happen to the data used by this adapter.
                \item \textbf{void	setAutofillOptions(CharSequence... options)}: Sets the value returned by getAutofillOptions()
                \item \textbf{void	unregisterDataSetObserver(DataSetObserver observer)}: Unregister an observer that has previously been registered with this adapter via registerDataSetObserver(DataSetObserver).
            \end{itemize}
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Hierarchy}
            \begin{center}
                java.lang.Object $\to$	android.widget.BaseAdapter $\to $	android.widget.ArrayAdapter<T>
            \end{center}
        \item \textbf{Include}
            \bigbreak \noindent 
            \begin{javacode}
        ...
        \item \textbf{Public methods}
            \begin{itemize}
                \item \textbf{void	add(T object)}: Adds the specified object at the end of the array.
                \item \textbf{void	addAll(Collection<? extends T> collection)}: Adds the specified Collection at the end of the array.
                \item \textbf{void	addAll(T... items)}: Adds the specified items at the end of the array.
                \item \textbf{void	clear()}: Remove all elements from the list.
                \item \textbf{static ArrayAdapter<CharSequence>	createFromResource(Context context, int textArrayResId, int textViewResId)}: Creates a new ArrayAdapter from external resources.
                \item \textbf{CharSequence[]	getAutofillOptions()}: Gets a string representation of the adapter data that can help AutofillService autofill the view backed by the adapter.
                \item \textbf{Context	getContext()}: Returns the context associated with this array adapter.
                \item \textbf{int	getCount()}: How many items are in the data set represented by this Adapter.
                \item \textbf{View	getDropDownView(int position, View convertView, ViewGroup parent)}: Gets a View that displays in the drop down popup the data at the specified position in the data set.
                \item \textbf{Resources.Theme	getDropDownViewTheme()}: Returns the value previously set by a call to setDropDownViewTheme(android.content.res.Resources.Theme).
                \item \textbf{Filter	getFilter()}: Returns a filter that can be used to constrain data with a filtering pattern.
                \item \textbf{T	getItem(int position)}: Get the data item associated with the specified position in the data set.
                \item \textbf{long	getItemId(int position)}: Get the row id associated with the specified position in the list.
                \item \textbf{int	getPosition(T item)}: Returns the position of the specified item in the array.
                \item \textbf{View	getView(int position, View convertView, ViewGroup parent)}: Get a View that displays the data at the specified position in the data set.
                \item \textbf{void	insert(T object, int index)}: Inserts the specified object at the specified index in the array.
                \item \textbf{void	notifyDataSetChanged()}: Notifies the attached observers that the underlying data has been changed and any View reflecting the data set should refresh itself.
                \item \textbf{void	remove(T object)}: Removes the specified object from the array.
                \item \textbf{void	setDropDownViewResource(int resource)}: Sets the layout resource to create the drop down views.
                \item \textbf{void	setDropDownViewTheme(Resources.Theme theme)}: Sets the Resources.Theme against which drop-down views are inflated.
                \item \textbf{void	setNotifyOnChange(boolean notifyOnChange)}: Control whether methods that change the list (add(T), addAll(java.util.Collection), addAll(java.lang.Object[]), insert(T, int), remove(T), clear(), sort(java.util.Comparator)) automatically call notifyDataSetChanged().
                \item \textbf{void	sort(Comparator<? super T> comparator)}: Sorts the content of this adapter using the specified comparator.
            \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Hierarchy}
            \begin{center}
                java.lang.Object $\to$	android.view.View $\to$	android.widget.TextView $\to$	android.widget.EditText $\to$	android.widget.AutoCompleteTextView
            \end{center}
        \item \textbf{Include}
            \bigbreak \noindent 
            \begin{javacode}
        ...
        \item \textbf{Public methods}
            \begin{itemize}
                \item \textbf{void	clearListSelection()}: Clear the list selection.
                \item \textbf{void	dismissDropDown()}: Closes the drop down if present on screen.
                \item \textbf{boolean	enoughToFilter()}: Returns true if the amount of text in the field meets or exceeds the getThreshold() requirement.
                \item \textbf{CharSequence	getAccessibilityClassName()}: Return the class name of this object to be used for accessibility purposes.
                \item \textbf{ListAdapter	getAdapter()}: Returns a filterable list adapter used for auto completion.
                \item \textbf{CharSequence	getCompletionHint()}: Gets the optional hint text displayed at the bottom of the the matching list.
                \item \textbf{int	getDropDownAnchor()}: Returns the id for the view that the auto-complete drop down list is anchored to.
                \item \textbf{Drawable	getDropDownBackground()}: Gets the background of the auto-complete drop-down list.
                \item \textbf{int	getDropDownHeight()}: Returns the current height for the auto-complete drop down list.
                \item \textbf{int	getDropDownHorizontalOffset()}: Gets the horizontal offset used for the auto-complete drop-down list.
                \item \textbf{int	getDropDownVerticalOffset()}: Gets the vertical offset used for the auto-complete drop-down list.
                \item \textbf{int	getDropDownWidth()}: Returns the current width for the auto-complete drop down list.
                \item \textbf{int	getInputMethodMode()}: Returns the input method mode used by the auto complete dropdown.
                \item \textbf{AdapterView.OnItemClickListener	getItemClickListener()}: This method was deprecated in API level 15. Use getOnItemClickListener() intead
                \item \textbf{AdapterView.OnItemSelectedListener	getItemSelectedListener()}: This method was deprecated in API level 15. Use getOnItemSelectedListener() intead
                \item \textbf{int	getListSelection()}: Get the position of the dropdown view selection, if there is one.
                \item \textbf{AdapterView.OnItemClickListener	getOnItemClickListener()}: Returns the listener that is notified whenever the user clicks an item in the drop down list.
                \item \textbf{AdapterView.OnItemSelectedListener	getOnItemSelectedListener()}: Returns the listener that is notified whenever the user selects an item in the drop down list.
                \item \textbf{int	getThreshold()}: Returns the number of characters the user must type before the drop down list is shown.
                \item \textbf{AutoCompleteTextView.Validator	getValidator()}: Returns the Validator set with setValidator(Validator), or null if it was not set.
                \item \textbf{boolean	isPerformingCompletion()}: Identifies whether the view is currently performing a text completion, so subclasses can decide whether to respond to text changed events.
                \item \textbf{boolean	isPopupShowing()}: Indicates whether the popup menu is showing.
                \item \textbf{void	onCommitCompletion(CompletionInfo completion)}: Called by the framework in response to a text completion from the current input method, provided by it calling InputConnection.commitCompletion().
                \item \textbf{void	onFilterComplete(int count)}: Notifies the end of a filtering operation.
                \item \textbf{boolean	onKeyDown(int keyCode, KeyEvent event)}: Default implementation of KeyEvent.Callback.onKeyDown(): perform press of the view when KeyEvent.KEYCODE\_DPAD\_CENTER or KeyEvent.KEYCODE\_ENTER is released, if the view is enabled and clickable.
                \item \textbf{boolean	onKeyPreIme(int keyCode, KeyEvent event)}: Handle a key event before it is processed by any input method associated with the view hierarchy.
                \item \textbf{boolean	onKeyUp(int keyCode, KeyEvent event)}: Default implementation of KeyEvent.Callback.onKeyUp(): perform clicking of the view when KeyEvent.KEYCODE\_DPAD\_CENTER, KeyEvent.KEYCODE\_ENTER or KeyEvent.KEYCODE\_SPACE is released.
                \item \textbf{void	onWindowFocusChanged(boolean hasWindowFocus)}: Called when the window containing this view gains or loses focus.
                \item \textbf{void	performCompletion()}: Performs the text completion by converting the selected item from the drop down list into a string, replacing the text box's content with this string and finally dismissing the drop down menu.
                \item \textbf{void	performValidation()}: If a validator was set on this view and the current string is not valid, ask the validator to fix it.
                \item \textbf{final void	refreshAutoCompleteResults()}: Refreshes the auto complete results.
                \item \textbf{<T extends ListAdapter & Filterable> void	setAdapter(T adapter)}: Changes the list of data used for auto completion.
                \item \textbf{void	setCompletionHint(CharSequence hint)}: Sets the optional hint text that is displayed at the bottom of the the matching list.
                \item \textbf{void	setDropDownAnchor(int id)}: Sets the view to which the auto-complete drop down list should anchor.
                \item \textbf{void	setDropDownBackgroundDrawable(Drawable d)}: Sets the background of the auto-complete drop-down list.
                \item \textbf{void	setDropDownBackgroundResource(int id)}: Sets the background of the auto-complete drop-down list.
                \item \textbf{void	setDropDownHeight(int height)}: Sets the current height for the auto-complete drop down list.
                \item \textbf{void	setDropDownHorizontalOffset(int offset)}: Sets the horizontal offset used for the auto-complete drop-down list.
                \item \textbf{void	setDropDownVerticalOffset(int offset)}: Sets the vertical offset used for the auto-complete drop-down list.
                \item \textbf{void	setDropDownWidth(int width)}: Sets the current width for the auto-complete drop down list.
                \item \textbf{void	setInputMethodMode(int mode)}: Use this method to specify when the IME should be displayed.
                \item \textbf{void	setListSelection(int position)}: Set the position of the dropdown view selection.
                \item \textbf{void	setOnClickListener(View.OnClickListener listener)}: Register a callback to be invoked when this view is clicked.
                \item \textbf{void	setOnDismissListener(AutoCompleteTextView.OnDismissListener dismissListener)}: Set a listener that will be invoked whenever the AutoCompleteTextView's list of completions is dismissed.
                \item \textbf{void	setOnItemClickListener(AdapterView.OnItemClickListener l)}: Sets the listener that will be notified when the user clicks an item in the drop down list.
                \item \textbf{void	setOnItemSelectedListener(AdapterView.OnItemSelectedListener l)}: Sets the listener that will be notified when the user selects an item in the drop down list.
                \item \textbf{void	setText(CharSequence text, boolean filter)}: Like TextView.setText(java.lang.CharSequence), except that it can disable filtering.
                \item \textbf{void	setThreshold(int threshold)}: Specifies the minimum number of characters the user has to type in the edit box before the drop down list is shown.
                \item \textbf{void	setValidator(AutoCompleteTextView.Validator validator)}: Sets the validator used to perform text validation.
                \item \textbf{void	showDropDown()}: Displays the drop down on screen.
            \end{itemize}
        \item \textbf{Protected methods}
            \begin{itemize}
                \item \textbf{CharSequence	convertSelectionToString(Object selectedItem)}: Converts the selected item from the drop down list into a sequence of character that can be used in the edit box.
                \item \textbf{Filter	getFilter()}: Returns the Filter obtained from Filterable.getFilter, or null if setAdapter(T) was not called with a Filterable.
                \item \textbf{void	onAttachedToWindow()}: This is called when the view is attached to a window.
                \item \textbf{void	onDetachedFromWindow()}: This is called when the view is detached from a window.
                \item \textbf{void	onDisplayHint(int hint)}: Gives this view a hint about whether is displayed or not.
                \item \textbf{void	onFocusChanged(boolean focused, int direction, Rect previouslyFocusedRect)}: Called by the view system when the focus state of this view changes.
                \item \textbf{void	performFiltering(CharSequence text, int keyCode)}: Starts filtering the content of the drop down list.
                \item \textbf{void	replaceText(CharSequence text)}: Performs the text completion by replacing the current text by the selected item.
                \item \textbf{boolean	setFrame(int l, int t, int r, int b)}:
            \end{itemize}
    \end{itemize}
    ...
    \pagebreak 
    \subsection{InputEvent (Abstract)}
    \begin{itemize}
        \item \textbf{Hierarchy}
            \begin{center}
               java.lang.Object $\to$	android.view.InputEvent
            \end{center}
        \item \textbf{Include}
            \bigbreak \noindent 
            \begin{javacode}
        ...
        \item \textbf{Public methods}
            \begin{itemize}
                \item \textbf{int	describeContents()}: Describe the kinds of special objects contained in this Parcelable instance's marshaled representation.
                \item \textbf{final InputDevice	getDevice()}: Gets the device that this event came from.
                \item \textbf{abstract int	getDeviceId()}: Gets the id for the device that this event came from.
                \item \textbf{abstract long	getEventTime()}: Retrieve the time this event occurred, in the SystemClock.uptimeMillis() time base.
                \item \textbf{abstract int	getSource()}: Gets the source of the event.
                \item \textbf{boolean	isFromSource(int source)}: Determines whether the event is from the given source.
            \end{itemize}
        \item \textbf{Fields}
            \begin{itemize}
                \item \textbf{public static final Creator<InputEvent>	CREATOR}:
            \end{itemize}
    \end{itemize}

    \pagebreak
    \subsection{MotionEvent (Extends InputEvent)}
    \begin{itemize}
        \item \textbf{Hierarchy}
            \begin{center}
                java.lang.Object $\to$	android.view.InputEvent $\to$	android.view.MotionEvent
            \end{center}
        \item \textbf{Include}
            \bigbreak \noindent 
            \begin{javacode}
        ...
        \item \textbf{Public methods}
            \begin{itemize}
                \item \textbf{static String	actionToString(int action)}: Returns a string that represents the symbolic name of the specified unmasked action such as "ACTION\_DOWN", "ACTION\_POINTER\_DOWN(3)" or an equivalent numeric constant such as "35" if unknown.
                \item \textbf{void	addBatch(long eventTime, PointerCoords[] pointerCoords, int metaState)}: Add a new movement to the batch of movements in this event.
                \item \textbf{void	addBatch(long eventTime, float x, float y, float pressure, float size, int metaState)}: Add a new movement to the batch of movements in this event.
                \item \textbf{static int	axisFromString(String symbolicName)}: Gets an axis by its symbolic name such as "AXIS\_X" or an equivalent numeric constant such as "42".
                \item \textbf{static String	axisToString(int axis)}: Returns a string that represents the symbolic name of the specified axis such as "AXIS\_X" or an equivalent numeric constant such as "42" if unknown.
                \item \textbf{int	findPointerIndex(int pointerId)}: Given a pointer identifier, find the index of its data in the event.
                \item \textbf{int	getAction()}: Return the kind of action being performed.
                \item \textbf{int	getActionButton()}: Gets which button has been modified during a press or release action.
                \item \textbf{int	getActionIndex()}: For ACTION\_POINTER\_DOWN or ACTION\_POINTER\_UP as returned by getActionMasked(), this returns the associated pointer index.
                \item \textbf{int	getActionMasked()}: Return the masked action being performed, without pointer index information.
                \item \textbf{float	getAxisValue(int axis, int pointerIndex)}: Returns the value of the requested axis for the given pointer index (use getPointerId(int) to find the pointer identifier for this index).
                \item \textbf{float	getAxisValue(int axis)}: getAxisValue(int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{int	getButtonState()}: Gets the state of all buttons that are pressed such as a mouse or stylus button.
                \item \textbf{int	getClassification()}: Returns the classification for the current gesture.
                \item \textbf{int	getDeviceId()}: Gets the id for the device that this event came from.
                \item \textbf{long	getDownTime()}: Returns the time (in ms) when the user originally pressed down to start a stream of position events.
                \item \textbf{int	getEdgeFlags()}: Returns a bitfield indicating which edges, if any, were touched by this MotionEvent.
                \item \textbf{long	getEventTime()}: Retrieve the time this event occurred, in the SystemClock.uptimeMillis() time base.
                \item \textbf{long	getEventTimeNanos()}: Retrieve the time this event occurred, in the SystemClock.uptimeMillis() time base but with nanosecond precision.
                \item \textbf{int	getFlags()}: Gets the motion event flags.
                \item \textbf{float	getHistoricalAxisValue(int axis, int pointerIndex, int pos)}: Returns the historical value of the requested axis, as per getAxisValue(int, int), occurred between this event and the previous event for the given pointer.
                \item \textbf{float	getHistoricalAxisValue(int axis, int pos)}: getHistoricalAxisValue(int, int, int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{long	getHistoricalEventTime(int pos)}: Returns the time that a historical movement occurred between this event and the previous event, in the SystemClock.uptimeMillis() time base.
                \item \textbf{long	getHistoricalEventTimeNanos(int pos)}: Returns the time that a historical movement occurred between this event and the previous event, in the SystemClock.uptimeMillis() time base but with nanosecond (instead of millisecond) precision.
                \item \textbf{float	getHistoricalOrientation(int pointerIndex, int pos)}: Returns a historical orientation coordinate, as per getOrientation(int), that occurred between this event and the previous event for the given pointer.
                \item \textbf{float	getHistoricalOrientation(int pos)}: getHistoricalOrientation(int, int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{void	getHistoricalPointerCoords(int pointerIndex, int pos, MotionEvent.PointerCoords outPointerCoords)}: Populates a PointerCoords object with historical pointer coordinate data, as per getPointerCoords(int, PointerCoords), that occurred between this event and the previous event for the given pointer.
                \item \textbf{float	getHistoricalPressure(int pointerIndex, int pos)}: Returns a historical pressure coordinate, as per getPressure(int), that occurred between this event and the previous event for the given pointer.
                \item \textbf{float	getHistoricalPressure(int pos)}: getHistoricalPressure(int, int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getHistoricalSize(int pos)}: getHistoricalSize(int, int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getHistoricalSize(int pointerIndex, int pos)}: Returns a historical size coordinate, as per getSize(int), that occurred between this event and the previous event for the given pointer.
                \item \textbf{float	getHistoricalToolMajor(int pos)}: getHistoricalToolMajor(int, int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getHistoricalToolMajor(int pointerIndex, int pos)}: Returns a historical tool major axis coordinate, as per getToolMajor(int), that occurred between this event and the previous event for the given pointer.
                \item \textbf{float	getHistoricalToolMinor(int pos)}: getHistoricalToolMinor(int, int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getHistoricalToolMinor(int pointerIndex, int pos)}: Returns a historical tool minor axis coordinate, as per getToolMinor(int), that occurred between this event and the previous event for the given pointer.
                \item \textbf{float	getHistoricalTouchMajor(int pos)}: getHistoricalTouchMajor(int, int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getHistoricalTouchMajor(int pointerIndex, int pos)}: Returns a historical touch major axis coordinate, as per getTouchMajor(int), that occurred between this event and the previous event for the given pointer.
                \item \textbf{float	getHistoricalTouchMinor(int pointerIndex, int pos)}: Returns a historical touch minor axis coordinate, as per getTouchMinor(int), that occurred between this event and the previous event for the given pointer.
                \item \textbf{float	getHistoricalTouchMinor(int pos)}: getHistoricalTouchMinor(int, int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getHistoricalX(int pos)}: getHistoricalX(int, int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getHistoricalX(int pointerIndex, int pos)}: Returns a historical X coordinate, as per getX(int), that occurred between this event and the previous event for the given pointer.
                \item \textbf{float	getHistoricalY(int pos)}: getHistoricalY(int, int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getHistoricalY(int pointerIndex, int pos)}: Returns a historical Y coordinate, as per getY(int), that occurred between this event and the previous event for the given pointer.
                \item \textbf{int	getHistorySize()}: Returns the number of historical points in this event.
                \item \textbf{int	getMetaState()}: Returns the state of any meta / modifier keys that were in effect when the event was generated.
                \item \textbf{float	getOrientation()}: getOrientation(int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getOrientation(int pointerIndex)}: Returns the value of AXIS\_ORIENTATION for the given pointer index.
                \item \textbf{void	getPointerCoords(int pointerIndex, MotionEvent.PointerCoords outPointerCoords)}: Populates a PointerCoords object with pointer coordinate data for the specified pointer index.
                \item \textbf{int	getPointerCount()}: The number of pointers of data contained in this event.
                \item \textbf{int	getPointerId(int pointerIndex)}: Return the pointer identifier associated with a particular pointer data index in this event.
                \item \textbf{void	getPointerProperties(int pointerIndex, MotionEvent.PointerProperties outPointerProperties)}: Populates a PointerProperties object with pointer properties for the specified pointer index.
                \item \textbf{float	getPressure(int pointerIndex)}: Returns the value of AXIS\_PRESSURE for the given pointer index.
                \item \textbf{float	getPressure()}: getPressure(int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getRawX()}: Returns the exact place on the screen where the finger is touching.
                \item \textbf{float	getRawX(int pointerIndex)}: Returns the X coordinate of the pointer referenced by pointerIndex for this motion event.
                \item \textbf{float	getRawY()}: Returns the exact place on the screen where the finger is touching.
                \item \textbf{float	getRawY(int pointerIndex)}: Returns the Y coordinate of the pointer referenced by pointerIndex for this motion event.
                \item \textbf{float	getSize(int pointerIndex)}: Returns the value of AXIS\_SIZE for the given pointer index.
                \item \textbf{float	getSize()}: getSize(int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{int	getSource()}: Gets the source of the event.
                \item \textbf{float	getToolMajor(int pointerIndex)}: Returns the value of AXIS\_TOOL\_MAJOR for the given pointer index.
                \item \textbf{float	getToolMajor()}: getToolMajor(int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getToolMinor()}: getToolMinor(int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getToolMinor(int pointerIndex)}: Returns the value of AXIS\_TOOL\_MINOR for the given pointer index.
                \item \textbf{int	getToolType(int pointerIndex)}: Gets the tool type of a pointer for the given pointer index.
                \item \textbf{float	getTouchMajor()}: getTouchMajor(int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getTouchMajor(int pointerIndex)}: Returns the value of AXIS\_TOUCH\_MAJOR for the given pointer index.
                \item \textbf{float	getTouchMinor(int pointerIndex)}: Returns the value of AXIS\_TOUCH\_MINOR for the given pointer index.
                \item \textbf{float	getTouchMinor()}: getTouchMinor(int) for the first pointer index (may be an arbitrary pointer identifier).
                \item \textbf{float	getX(int pointerIndex)}: Returns the X coordinate of the pointer referenced by pointerIndex for this motion event.
                \item \textbf{float	getX()}: Exact place relative to the top-left corner of the View receiving the event 
                \item \textbf{float	getXPrecision()}: Return the precision of the X coordinates being reported.
                \item \textbf{float	getY()}: Exact place relative to the top-left corner of the View receiving the event 
                \item \textbf{float	getY(int pointerIndex)}: Returns the Y coordinate of the pointer referenced by pointerIndex for this motion event.
                \item \textbf{float	getYPrecision()}: Return the precision of the Y coordinates being reported.
                \item \textbf{boolean	isButtonPressed(int button)}: Checks if a mouse or stylus button (or combination of buttons) is pressed.
                \item \textbf{static MotionEvent	obtain(long downTime, long eventTime, int action, float x, float y, int metaState)}: Create a new MotionEvent, filling in a subset of the basic motion values.
                \item \textbf{static MotionEvent	obtain(long downTime, long eventTime, int action, int pointerCount, float x, float y, float pressure, float size, int metaState, float xPrecision, float yPrecision, int deviceId, int edgeFlags)}: This method was deprecated in API level 15. Use obtain(long, long, int, float, float, float, float, int, float, float, int, int) instead.
                \item \textbf{static MotionEvent	obtain(long downTime, long eventTime, int action, int pointerCount, int[] pointerIds, PointerCoords[] pointerCoords, int metaState, float xPrecision, float yPrecision, int deviceId, int edgeFlags, int source, int flags)}: This method was deprecated in API level 15. Use obtain(long, long, int, int, android.view.MotionEvent.PointerProperties[], android.view.MotionEvent.PointerCoords[], int, int, float, float, int, int, int, int) instead.
                \item \textbf{static MotionEvent	obtain(long downTime, long eventTime, int action, int pointerCount, PointerProperties[] pointerProperties, PointerCoords[] pointerCoords, int metaState, int buttonState, float xPrecision, float yPrecision, int deviceId, int edgeFlags, int source, int flags)}: Create a new MotionEvent, filling in all of the basic values that define the motion.
                \item \textbf{static MotionEvent	obtain(MotionEvent other)}: Create a new MotionEvent, copying from an existing one.
                \item \textbf{static MotionEvent	obtain(long downTime, long eventTime, int action, float x, float y, float pressure, float size, int metaState, float xPrecision, float yPrecision, int deviceId, int edgeFlags)}: Create a new MotionEvent, filling in all of the basic values that define the motion.
                ...
                \item \textbf{static MotionEvent	obtainNoHistory(MotionEvent other)}: Create a new MotionEvent, copying from an existing one, but not including any historical point information.
                \item \textbf{void	offsetLocation(float deltaX, float deltaY)}: Adjust this event's location.
                \item \textbf{void	recycle()}: Recycle the MotionEvent, to be re-used by a later caller.
                \item \textbf{void	setAction(int action)}: Sets this event's action.
                \item \textbf{void	setEdgeFlags(int flags)}: Sets the bitfield indicating which edges, if any, were touched by this MotionEvent.
                \item \textbf{void	setLocation(float x, float y)}: Set this event's location.
                \item \textbf{void	setSource(int source)}:
                \item \textbf{String	toString()}: Returns a string representation of the object.
                \item \textbf{void	transform(Matrix matrix)}: Applies a transformation matrix to all of the points in the event.
                \item \textbf{void	writeToParcel(Parcel out, int flags)}: Flatten this object in to a Parcel.
            \end{itemize}
        \item \textbf{Protected methods}
            \begin{itemize}
                \item \textbf{void	finalize()}: Called by the garbage collector on an object when garbage collection determines that there are no more references to the object.
        ...
        \item \textbf{Fields}
            \begin{itemize}
                \item \textbf{public static final Creator<MotionEvent>	CREATOR}:
            \end{itemize}
        \item \textbf{Constants}
            \begin{itemize}
                \item \textbf{int	ACTION\_BUTTON\_PRESS}: Constant for getActionMasked(): A button has been pressed.
                \item \textbf{int	ACTION\_BUTTON\_RELEASE}: Constant for getActionMasked(): A button has been released.
                \item \textbf{int	ACTION\_CANCEL}: Constant for getActionMasked(): The current gesture has been aborted.
                \item \textbf{int	ACTION\_DOWN}: Constant for getActionMasked(): A pressed gesture has started, the motion contains the initial starting location.
                \item \textbf{int	ACTION\_HOVER\_ENTER}: Constant for getActionMasked(): The pointer is not down but has entered the boundaries of a window or view.
                \item \textbf{int	ACTION\_HOVER\_EXIT}: Constant for getActionMasked(): The pointer is not down but has exited the boundaries of a window or view.
                \item \textbf{int	ACTION\_HOVER\_MOVE}: Constant for getActionMasked(): A change happened but the pointer is not down (unlike ACTION\_MOVE).
                \item \textbf{int	ACTION\_MASK}: Bit mask of the parts of the action code that are the action itself.
                \item \textbf{int	ACTION\_MOVE}: Constant for getActionMasked(): A change has happened during a press gesture (between ACTION\_DOWN and ACTION\_UP).
                \item \textbf{int	ACTION\_OUTSIDE}: Constant for getActionMasked(): A movement has happened outside of the normal bounds of the UI element.
                \item \textbf{int	ACTION\_POINTER\_1\_DOWN}: This constant was deprecated in API level 15. Use ACTION\_POINTER\_INDEX\_MASK to retrieve the data index associated with ACTION\_POINTER\_DOWN.
                \item \textbf{int	ACTION\_POINTER\_1\_UP}: This constant was deprecated in API level 15. Use ACTION\_POINTER\_INDEX\_MASK to retrieve the data index associated with ACTION\_POINTER\_UP.
                \item \textbf{int	ACTION\_POINTER\_2\_DOWN}: This constant was deprecated in API level 15. Use ACTION\_POINTER\_INDEX\_MASK to retrieve the data index associated with ACTION\_POINTER\_DOWN.
                \item \textbf{int	ACTION\_POINTER\_2\_UP}: This constant was deprecated in API level 15. Use ACTION\_POINTER\_INDEX\_MASK to retrieve the data index associated with ACTION\_POINTER\_UP.
                \item \textbf{int	ACTION\_POINTER\_3\_DOWN}: This constant was deprecated in API level 15. Use ACTION\_POINTER\_INDEX\_MASK to retrieve the data index associated with ACTION\_POINTER\_DOWN.
                \item \textbf{int	ACTION\_POINTER\_3\_UP}: This constant was deprecated in API level 15. Use ACTION\_POINTER\_INDEX\_MASK to retrieve the data index associated with ACTION\_POINTER\_UP.
                \item \textbf{int	ACTION\_POINTER\_DOWN}: Constant for getActionMasked(): A non-primary pointer has gone down.
                \item \textbf{int	ACTION\_POINTER\_ID\_MASK}: This constant was deprecated in API level 15. Renamed to ACTION\_POINTER\_INDEX\_MASK to match the actual data contained in these bits.
                \item \textbf{int	ACTION\_POINTER\_ID\_SHIFT}: This constant was deprecated in API level 15. Renamed to ACTION\_POINTER\_INDEX\_SHIFT to match the actual data contained in these bits.
                \item \textbf{int	ACTION\_POINTER\_INDEX\_MASK}: Bits in the action code that represent a pointer index, used with ACTION\_POINTER\_DOWN and ACTION\_POINTER\_UP.
                \item \textbf{int	ACTION\_POINTER\_INDEX\_SHIFT}: Bit shift for the action bits holding the pointer index as defined by ACTION\_POINTER\_INDEX\_MASK.
                \item \textbf{int	ACTION\_POINTER\_UP}: Constant for getActionMasked(): A non-primary pointer has gone up.
                \item \textbf{int	ACTION\_SCROLL}: Constant for getActionMasked(): The motion event contains relative vertical and/or horizontal scroll offsets.
                \item \textbf{int	ACTION\_UP}: Constant for getActionMasked(): A pressed gesture has finished, the motion contains the final release location as well as any intermediate points since the last down or move event.
                \item \textbf{int	AXIS\_BRAKE}: Axis constant: Brake axis of a motion event.
                \item \textbf{int	AXIS\_DISTANCE}: Axis constant: Distance axis of a motion event.
                \item \textbf{int	AXIS\_GAS}: Axis constant: Gas axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_1}: Axis constant: Generic 1 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_10}: Axis constant: Generic 10 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_11}: Axis constant: Generic 11 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_12}: Axis constant: Generic 12 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_13}: Axis constant: Generic 13 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_14}: Axis constant: Generic 14 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_15}: Axis constant: Generic 15 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_16}: Axis constant: Generic 16 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_2}: Axis constant: Generic 2 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_3}: Axis constant: Generic 3 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_4}: Axis constant: Generic 4 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_5}: Axis constant: Generic 5 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_6}: Axis constant: Generic 6 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_7}: Axis constant: Generic 7 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_8}: Axis constant: Generic 8 axis of a motion event.
                \item \textbf{int	AXIS\_GENERIC\_9}: Axis constant: Generic 9 axis of a motion event.
                \item \textbf{int	AXIS\_GESTURE\_PINCH\_SCALE\_FACTOR}: Axis constant: pinch scale factor of a motion event.
                \item \textbf{int	AXIS\_GESTURE\_SCROLL\_X\_DISTANCE}: Axis constant: X scroll distance axis of a motion event.
                \item \textbf{int	AXIS\_GESTURE\_SCROLL\_Y\_DISTANCE}: Axis constant: Y scroll distance axis of a motion event.
                \item \textbf{int	AXIS\_GESTURE\_X\_OFFSET}: Axis constant: X gesture offset axis of a motion event.
                \item \textbf{int	AXIS\_GESTURE\_Y\_OFFSET}: Axis constant: Y gesture offset axis of a motion event.
                \item \textbf{int	AXIS\_HAT\_X}: Axis constant: Hat X axis of a motion event.
                \item \textbf{int	AXIS\_HAT\_Y}: Axis constant: Hat Y axis of a motion event.
                \item \textbf{int	AXIS\_HSCROLL}: Axis constant: Horizontal Scroll axis of a motion event.
                \item \textbf{int	AXIS\_LTRIGGER}: Axis constant: Left Trigger axis of a motion event.
                \item \textbf{int	AXIS\_ORIENTATION}: Axis constant: Orientation axis of a motion event.
                \item \textbf{int	AXIS\_PRESSURE}: Axis constant: Pressure axis of a motion event.
                \item \textbf{int	AXIS\_RELATIVE\_X}: Axis constant: The movement of x position of a motion event.
                \item \textbf{int	AXIS\_RELATIVE\_Y}: Axis constant: The movement of y position of a motion event.
                \item \textbf{int	AXIS\_RTRIGGER}: Axis constant: Right Trigger axis of a motion event.
                \item \textbf{int	AXIS\_RUDDER}: Axis constant: Rudder axis of a motion event.
                \item \textbf{int	AXIS\_RX}: Axis constant: X Rotation axis of a motion event.
                \item \textbf{int	AXIS\_RY}: Axis constant: Y Rotation axis of a motion event.
                \item \textbf{int	AXIS\_RZ}: Axis constant: Z Rotation axis of a motion event.
                \item \textbf{int	AXIS\_SCROLL}: Axis constant: Generic scroll axis of a motion event.
                \item \textbf{int	AXIS\_SIZE}: Axis constant: Size axis of a motion event.
                \item \textbf{int	AXIS\_THROTTLE}: Axis constant: Throttle axis of a motion event.
                \item \textbf{int	AXIS\_TILT}: Axis constant: Tilt axis of a motion event.
                \item \textbf{int	AXIS\_TOOL\_MAJOR}: Axis constant: ToolMajor axis of a motion event.
                \item \textbf{int	AXIS\_TOOL\_MINOR}: Axis constant: ToolMinor axis of a motion event.
                \item \textbf{int	AXIS\_TOUCH\_MAJOR}: Axis constant: TouchMajor axis of a motion event.
                \item \textbf{int	AXIS\_TOUCH\_MINOR}: Axis constant: TouchMinor axis of a motion event.
                \item \textbf{int	AXIS\_VSCROLL}: Axis constant: Vertical Scroll axis of a motion event.
                \item \textbf{int	AXIS\_WHEEL}: Axis constant: Wheel axis of a motion event.
                \item \textbf{int	AXIS\_X}: Axis constant: X axis of a motion event.
                \item \textbf{int	AXIS\_Y}: Axis constant: Y axis of a motion event.
                \item \textbf{int	AXIS\_Z}: Axis constant: Z axis of a motion event.
                \item \textbf{int	BUTTON\_BACK}: Button constant: Back button pressed (mouse back button).
                \item \textbf{int	BUTTON\_FORWARD}: Button constant: Forward button pressed (mouse forward button).
                \item \textbf{int	BUTTON\_PRIMARY}: Button constant: Primary button (left mouse button).
                \item \textbf{int	BUTTON\_SECONDARY}: Button constant: Secondary button (right mouse button).
                \item \textbf{int	BUTTON\_STYLUS\_PRIMARY}: Button constant: Primary stylus button pressed.
                \item \textbf{int	BUTTON\_STYLUS\_SECONDARY}: Button constant: Secondary stylus button pressed.
                \item \textbf{int	BUTTON\_TERTIARY}: Button constant: Tertiary button (middle mouse button).
                \item \textbf{int	CLASSIFICATION\_AMBIGUOUS\_GESTURE}: Classification constant: Ambiguous gesture.
                \item \textbf{int	CLASSIFICATION\_DEEP\_PRESS}: Classification constant: Deep press.
                \item \textbf{int	CLASSIFICATION\_NONE}: Classification constant: None.
                \item \textbf{int	CLASSIFICATION\_PINCH}: Classification constant: touchpad pinch.
                \item \textbf{int	CLASSIFICATION\_TWO\_FINGER\_SWIPE}: Classification constant: touchpad scroll.
                \item \textbf{int	EDGE\_BOTTOM}: Flag indicating the motion event intersected the bottom edge of the screen.
                \item \textbf{int	EDGE\_LEFT}: Flag indicating the motion event intersected the left edge of the screen.
                \item \textbf{int	EDGE\_RIGHT}: Flag indicating the motion event intersected the right edge of the screen.
                \item \textbf{int	EDGE\_TOP}: Flag indicating the motion event intersected the top edge of the screen.
                \item \textbf{int	FLAG\_CANCELED}: This flag is only set for events with ACTION\_POINTER\_UP and ACTION\_CANCEL.
                \item \textbf{int	FLAG\_WINDOW\_IS\_OBSCURED}: This flag indicates that the window that received this motion event is partly or wholly obscured by another visible window above it and the event directly passed through the obscured area.
                \item \textbf{int	FLAG\_WINDOW\_IS\_PARTIALLY\_OBSCURED}: This flag indicates that the window that received this motion event is partly or wholly obscured by another visible window above it and the event did not directly pass through the obscured area.
                \item \textbf{int	INVALID\_POINTER\_ID}: An invalid pointer id.
                \item \textbf{int	TOOL\_TYPE\_ERASER}: Tool type constant: The tool is an eraser or a stylus being used in an inverted posture.
                \item \textbf{int	TOOL\_TYPE\_FINGER}: Tool type constant: The tool is a finger.
                \item \textbf{int	TOOL\_TYPE\_MOUSE}: Tool type constant: The tool is a mouse.
                \item \textbf{int	TOOL\_TYPE\_STYLUS}: Tool type constant: The tool is a stylus.
                \item \textbf{int	TOOL\_TYPE\_UNKNOWN}: Tool type constant: Unknown tool type.
            \end{itemize}
    \end{itemize}


    \pagebreak
    \subsection{View.OnTouchListener (Interface)}
    \begin{itemize}
        \item \textbf{Signature}
            \bigbreak \noindent 
            \begin{javacode}
        ...
        \item \textbf{Abstract methods}
            \begin{itemize}
                \item \textbf{abstract boolean	onTouch(View v, MotionEvent event)}: Called when a touch event is dispatched to a view.
            \end{itemize}
    \end{itemize}


    \pagebreak
    \subsection{GestureDetector}
    \begin{itemize}
        \item \textbf{Hierarchy}
            \begin{center}
                java.lang.Object $\to$	android.view.GestureDetector
            \end{center}
        \item \textbf{Include}
            \bigbreak \noindent 
            \begin{javacode}
        ...
        \item \textbf{Public methods}
            \begin{itemize}
                \item \textbf{boolean	isLongpressEnabled()}:
                \item \textbf{boolean	onGenericMotionEvent(MotionEvent ev)}: Analyzes the given generic motion event and if applicable triggers the appropriate callbacks on the OnGestureListener supplied.
                \item \textbf{boolean	onTouchEvent(MotionEvent ev)}: Analyzes the given motion event and if applicable triggers the appropriate callbacks on the OnGestureListener supplied.
                \item \textbf{void	setContextClickListener(GestureDetector.OnContextClickListener onContextClickListener)}: Sets the listener which will be called for context clicks.
                \item \textbf{void	setIsLongpressEnabled(boolean isLongpressEnabled)}: Set whether longpress is enabled, if this is enabled when a user presses and holds down you get a longpress event and nothing further.
                \item \textbf{void	setOnDoubleTapListener(GestureDetector.OnDoubleTapListener onDoubleTapListener)}: Sets the listener which will be called for double-tap and related gestures.
            \end{itemize}
    \end{itemize}

    \pagebreak
    \subsection{GestureDetector.OnGestureListener (Interface)}
    \begin{itemize}
        \item \textbf{Signature}
            \bigbreak \noindent 
            \begin{javacode}
        ...
        \item \textbf{Abstract methods}
            \begin{itemize}
                \item \textbf{abstract boolean	onDown(MotionEvent e)}: Notified when a tap occurs with the down MotionEvent that triggered it.
                \item \textbf{abstract boolean	onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY)}: Notified of a fling event when it occurs with the initial on down MotionEvent and the matching up MotionEvent.
                \item \textbf{abstract void	onLongPress(MotionEvent e)}: Notified when a long press occurs with the initial on down MotionEvent that trigged it.
                \item \textbf{abstract boolean	onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY)}: Notified when a scroll occurs with the initial on down MotionEvent and the current move MotionEvent.
                \item \textbf{abstract void	onShowPress(MotionEvent e)}: The user has performed a down MotionEvent and not performed a move or up yet.
                \item \textbf{abstract boolean	onSingleTapUp(MotionEvent e)}: Notified when a tap occurs with the up MotionEvent that triggered it.
            \end{itemize}
    \end{itemize}


    \pagebreak
    \subsection{GestureDetector.OnDoubleTapListener (Interface)}
    \begin{itemize}
        \item \textbf{Signature}
            \bigbreak \noindent 
            \begin{javacode}
        ...
        \item \textbf{Abstract methods}
            \begin{itemize}
                \item \textbf{abstract boolean	onDoubleTap(MotionEvent e)}: Notified when a double-tap occurs.
                \item \textbf{abstract boolean	onDoubleTapEvent(MotionEvent e)}: Notified when an event within a double-tap gesture occurs, including the down, move, and up events.
                \item \textbf{abstract boolean	onSingleTapConfirmed(MotionEvent e)}: Notified when a single-tap occurs.
            \end{itemize}
    \end{itemize}

    \pagebreak
    \subsection{GestureDetector.SimpleOnGestureListener}
    \begin{itemize}
        \item \textbf{Hierarchy}
            \bigbreak \noindent 
            \begin{javacode}
               java.lang.Object $\to$	android.view.GestureDetector.SimpleOnGestureListener 
            \end{javacode}
        \item \textbf{Include}
            \bigbreak \noindent
            \begin{javacode}
        ...
        \item \textbf{Public methods}
            \begin{itemize}
                \item \textbf{boolean	onContextClick(MotionEvent e)}: Notified when a context click occurs.
                \item \textbf{boolean	onDoubleTap(MotionEvent e)}: Notified when a double-tap occurs.
                \item \textbf{boolean	onDoubleTapEvent(MotionEvent e)}: Notified when an event within a double-tap gesture occurs, including the down, move, and up events.
                \item \textbf{boolean	onDown(MotionEvent e)}: Notified when a tap occurs with the down MotionEvent that triggered it.
                \item \textbf{boolean	onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY)}: Notified of a fling event when it occurs with the initial on down MotionEvent and the matching up MotionEvent.
                \item \textbf{void	onLongPress(MotionEvent e)}: Notified when a long press occurs with the initial on down MotionEvent that trigged it.
                \item \textbf{boolean	onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY)}: Notified when a scroll occurs with the initial on down MotionEvent and the current move MotionEvent.
                \item \textbf{void	onShowPress(MotionEvent e)}: The user has performed a down MotionEvent and not performed a move or up yet.
                \item \textbf{boolean	onSingleTapConfirmed(MotionEvent e)}: Notified when a single-tap occurs.
                \item \textbf{boolean	onSingleTapUp(MotionEvent e)}: Notified when a tap occurs with the up MotionEvent that triggered it.
            \end{itemize}
    \end{itemize}
