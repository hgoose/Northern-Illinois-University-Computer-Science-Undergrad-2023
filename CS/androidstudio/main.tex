...
\pagebreak 
    ...
    \begin{itemize}
        ...
        \item \textbf{How MeasureSpecs Are Used}: During layout:
            ...
            \end{enumerate}
        \item \textbf{view.measure}: measure() is a public final method on View that:
            \begin{enumerate}
                \item Receives two MeasureSpecs from the parent (width and height).
                \item Invokes the view’s onMeasure() method.
                \item Ensures the view sets its internal measured width and height.
                \item Enforces that the measurement process is valid.
            \end{enumerate}
            This method is part of phase 1 of the Android UI pipeline:
            \begin{align*}
                \text{measure } \to \text{ layout } \to \text{ draw }
            .\end{align*}
            \begin{itemize}
                \item Measurement defines how big a view wants to be.
                \item Layout defines where a view is placed.
                \item Draw defines how a view is rendered.
            \end{itemize}
            \bigbreak \noindent 
            \begin{javacode}
                public final void measure(int widthMeasureSpec, int heightMeasureSpec) {
                    // Save the measure specs
                    mMeasureCache or fields updated

                    // Call developer-overridable measurement function
                    onMeasure(widthMeasureSpec, heightMeasureSpec);

                    // After onMeasure returns, the view MUST have called
                    // setMeasuredDimension(w, h);

                    // Constraint enforcement
                    if (the view violated spec rules) {
                        correct or throw
                    }
                }
            \end{javacode}
