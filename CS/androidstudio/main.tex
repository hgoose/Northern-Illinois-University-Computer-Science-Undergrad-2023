...
\begin{document}
    ...
    \begin{itemize}
        ...
        \item \textbf{Other Common Built-In Layouts}:
            ...
            \end{itemize}
        \item \textbf{android.R.id.content}: special, system-defined ID that refers to the root view of your Activity’s window—specifically, the top-level ViewGroup that holds everything inside your Activity’s UI.
            \bigbreak \noindent 
            ...
            \end{javacode}
            \bigbreak \noindent 
            Puts the root layout for activity\_main in the \texttt{layout} variable.
        \item \textbf{Unit argument in java}: When a method like TextViews \texttt{setTextSize(int unit, float size)} requires a unit parameter, we can used \textbf{TypedValue} constants like
            \begin{itemize}
...
\pagebreak 
    ...
    \begin{itemize}
        ...
        \item \textbf{Base theme:} 
            ...
            \end{xmlcode}
        \item \textbf{GestureDetector.OnGestureListener}
            \bigbreak \noindent 
            \begin{javacode}
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv.setText("Touch and scroll or fling");
        tv.setTextSize(30);
        setContentView(tv);

        // Create the GestureDetector using THIS as the listener
        gestureDetector = new GestureDetector(this, this);
    }

    // Forward all MotionEvents to the GestureDetector
    @Override
    ...
    public boolean onDown(MotionEvent e) {
        Log.w(TAG, "onDown");
        return true;  // must return true to continue receiving events
    }
    ...
    \begin{itemize}
        \item \textbf{What is it}: InputEvent is the abstract base class for all low-level input events in Android. It represents any event generated by an input device.
        \item \textbf{Characteristics}
            \begin{itemize}
                \item It is not instantiated directly.
                \item It provides a unified parent type for different categories of input events.
                \item Motion-related and key-related events both derive from it.
            \end{itemize}
        \item \textbf{Two concrete subclasses}: 
            \begin{enumerate}
                \item \textbf{MotionEvent}: For touch, hover, scroll, and pointer movements.
                \item \textbf{KeyEvent}: For hardware key presses (keyboard, volume keys, etc.).
            \end{enumerate}
        \item \textbf{What InputEvent provides}: A unique event ID (getDeviceId()), methods for input device querying (e.g., source type, device capabilities), and generic event metadata.
    \end{itemize}

    \pagebreak
    \subsection{MotionEvent}
    \begin{itemize}
        \item \textbf{What is it}: MotionEvent represents all pointer-based activity on the screen. This includes touch, drag, swipe, pinch, hover, and multi-touch events.
        \item \textbf{What MotionEvent contains}: 
            \begin{itemize}
                \item \textbf{Action type}: Indicates what the user did. Constants like \texttt{ACTION\_DOWN}, \texttt{ACTION\_UP}, \texttt{ACTION\_MOVES}, etc.
                \item \textbf{Coordinates}: The event provides the touch coordinates:
                    \begin{itemize}
                        \item \textbf{getRawX() / getRawY()}: Screen coordinates
                        \item \textbf{getX() / getY()}: Coordinates relative to the view receiving the event
                    \end{itemize}
            \end{itemize}
        \item \textbf{Getting MotionEvents}: In Android, you never create a MotionEvent manually. The system generates it and passes it into specific callback methods.
            \begin{itemize}
                \item \textbf{onTouch(View v, MotionEvent event)}: Used with View.OnTouchListener. 
                \item \textbf{onTouchEvent(MotionEvent event)}: Every View has this method. Override it when you want the view itself to process touches.
                \item \textbf{dispatchTouchEvent(MotionEvent event)}: Called before onTouchEvent. Used rarely, but can intercept events at the view level.
            \end{itemize}
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Gestures}: A gesture is a pattern of touch movement performed by the user on the screen that the system recognizes as a meaningful interaction. It is \textbf{not} just a raw touch event.
            \bigbreak \noindent 
            ...
            \begin{itemize}
                ...
                \item Long press
                \item Scroll (drag)
                \item Fling (fast swipe)
                \item Pinch/zoom (multi-touch)
            \end{itemize}
        \item \textbf{MotionEvent vs Gesture}: A MotionEvent is just made up of 
            \begin{itemize}
                \item Raw data: coordinates, finger ID, action code
                \item Low-level
                \item One event at a time
                \item Examples: ACTION\_DOWN, ACTION\_MOVE, ACTION\_UP
            \end{itemize}
            Whereas a Gesture is 
            \begin{itemize}
                \item A meaningful pattern formed from many MotionEvents
                \item High-level
                \item Recognized by timing, distance, velocity
                \item Examples: “single tap”, “fling”, “scroll”, “double tap”
            \end{itemize}
        \item \textbf{Single tap, scroll, fling}: A single tap gesture usually consists of:
            \begin{enumerate}
                \item ACTION\_DOWN
                \item Short time interval
                \item ACTION\_UP
                \item Minimal movement
            \end{enumerate}
            A scroll gesture consists of:
            \begin{itemize}
                \item ACTION\_DOWN
                \item Many ACTION\_MOVE events
                \item ACTION\_UP
            \end{itemize}
            A fling gesture consists of:
            \begin{itemize}
                \item ACTION\_DOWN
                \item ACTION\_MOVE events
                \item ACTION\_UP with high velocity
            \end{itemize}

        \item \textbf{What is it}: GestureDetector is an Android framework class that analyzes a stream of MotionEvent objects and determines whether they form a gesture, such as:
            \begin{itemize}
                \item Single tap
                \item Double tap
                \item Long press
                \item Scroll
                \item Fling (quick swipe)
            \end{itemize}
            It removes the burden of manually interpreting touch sequences. In other words, you feed MotionEvents into GestureDetector, and it calls back into your code with high-level gesture events
        \item \textbf{Why GestureDetector}: Raw MotionEvents give you very low-level data:
            \begin{itemize}
                \item Coordinates
                \item Pressure
                \item Pointer index
                \item Timestamps
                \item Action codes (DOWN, MOVE, UP)
            \end{itemize}
            However, recognizing a gesture like “double tap” or “fling” requires:
            \begin{itemize}
                \item Timing analysis
                \item Movement thresholds
                \item Velocity computation
                \item Distinguishing tap vs scroll
                \item Multi-event patterns
            \end{itemize}
            GestureDetector handles all of this internally.
        \item \textbf{How it works}: There are three components:
            \begin{enumerate}
                \item \textbf{A listener object}: You implement one or both listener interfaces (GestureDetector.OnGestureListener (gestures)) (GestureDetector.OnDoubleTapListener (taps)). These contain multiple callback methods.
                \item \textbf{A GestureDetector instance}: You create it in your Activity or View:
                    \bigbreak \noindent 
                    \begin{javacode}
                        gestureDetector = new GestureDetector(this, listener);
                    \end{javacode}
                \item \textbf{Feeding MotionEvents}: Inside your Activity’s onTouchEvent() or a View’s touch listener, you pass the MotionEvent to GestureDetector:
                    \bigbreak \noindent 
                    \begin{javacode}
                        @Override
                        public boolean onTouchEvent(MotionEvent event) {
        ...
        \item \textbf{GestureDetector.onTouchEvent(MotionEvent event)}:   GestureDetector.onTouchEvent(MotionEvent event) is the core method you call to feed touch input into a GestureDetector. It is the entry point that allows the GestureDetector to examine all MotionEvents and decide whether they form a gesture (tap, scroll, fling, long-press, double-tap, etc.).
            \bigbreak \noindent 
            Its purpose is to analyze a MotionEvent and determine whether it represents a gesture. You call this method from your Activity or View whenever you receive MotionEvents.
            \bigbreak \noindent 
            \begin{javacode}
                @Override
                public boolean onTouchEvent(MotionEvent event) {
            ...
            \begin{enumerate}
                \item GestureDetector reads the event type
                \item It updates its internal gesture state
                \item It checks whether the event sequence matches a known gesture, if so, it calls the appropriate callback on your listener:
            \end{enumerate}
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{What is it}: GestureDetector.SimpleOnGestureListener is a convenience class that implements both:
            \begin{itemize}
                \item GestureDetector.OnGestureListener
                \item GestureDetector.OnDoubleTapListener
            \end{itemize}
        ...
        \item \textbf{Available methods}: Because it implements both gesture interfaces, you may selectively override any of the following:
            \begin{itemize}
                \item onDown(MotionEvent e)
                \item onShowPress(MotionEvent e)
                \item onSingleTapUp(MotionEvent e)
                \item onSingleTapConfirmed(MotionEvent e)
                \item onDoubleTap(MotionEvent e)
                \item onDoubleTapEvent(MotionEvent e)
                \item onLongPress(MotionEvent e)
                \item onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY)
                \item onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY)
            \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{What is it}: 
            A MeasureSpec is a compact instruction that a parent ViewGroup gives to a child View during the measurement phase of layout. It tells the child:
            \begin{itemize}
                \item How much space it may or must use, and
                \item How strictly that space must be followed.
            \end{itemize}
            ...
            \begin{itemize}
                \item \textbf{A mode}: Describes how the parent wants the child to size itself.
                \item \textbf{A size}: The pixel value associated with the constraint.
            \end{itemize}
            Together
            \begin{align*}
                \text{MeasureSpec } = (\text{mode } << 30) \mid \text{ size}
            .\end{align*}
            This is why MeasureSpec is passed around as a plain int.
        \item \textbf{Modes}: These modes define the constraint type.
            \begin{enumerate}
                \item \textbf{EXACTLY}: “You must be exactly this size.”
                    \bigbreak \noindent 
                    Produced when:
                    \begin{itemize}
                        \item The child uses match\_parent
                        \item The child uses a fixed value like 100dp
                    \end{itemize}
                    The child must report that exact dimension in measurement.
                \item \textbf{AT\_MOST}: “You can be any size you want, up to this maximum.”
                    \bigbreak \noindent 
                    Produced mainly when:
                    \begin{itemize}
                        \item The child uses wrap\_content
                        \item The parent can offer some remaining space (e.g. inside LinearLayout, FrameLayout)       
                    \end{itemize}
                    Child measures its intrinsic content but clamps to the max.
                \item \textbf{UNSPECIFIED}: “There is no upper bound. Do whatever your content requires.”
                    \bigbreak \noindent 
                    Occurs in special cases:
                    \begin{itemize}
                        \item A ScrollView measuring its child
                        \item Internal platform commands
                        \item Custom views that need unrestricted measurement
                    \end{itemize}
            \end{enumerate}
        \item \textbf{How MeasureSpecs Are Used}: During layout:
            \begin{enumerate}
                \item Parent receives its own MeasureSpecs from its parent.
                \item Parent computes new MeasureSpecs for each child.
                \item Parent calls:
                    \bigbreak \noindent 
                    \begin{javacode}
                     child.measure(widthMeasureSpec, heightMeasureSpec);
                    \end{javacode}
                \item Child’s onMeasure() reads the MeasureSpec:
                    \bigbreak \noindent 
                    \begin{javacode}
                        int mode = MeasureSpec.getMode(widthSpec);
                        int size = MeasureSpec.getSize(widthSpec);
                    \end{javacode}
                \item Child computes its measured width and height using:
                    \bigbreak \noindent 
                    \begin{javacode}
            ...
            \end{enumerate}
        \item \textbf{view.measure}: When we call
            \bigbreak \noindent 
            \begin{javacode}
            ...
            \begin{enumerate}
                \item Android executes onMeasure() for that view.
                \item The view recalculates its measured width/height.
                \item The view stores those values internally by calling:
                    \bigbreak \noindent 
                    \begin{javacode}
            ...
            onMeasure()... 
            \begin{itemize}
                \item Interprets the MeasureSpec constraints.
                \item Decide how large the view wants to be.
                \item Compute the final measured width/height.
            \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{RadioGroup.OnCheckedChangeListener}
            ...
            \end{javacode}
        \item \textbf{View.OnTouchListener}: View.OnTouchListener is an interface in the Android SDK that allows your code to receive touch events occurring on a specific View. 
            \bigbreak \noindent 
            ...
            \begin{itemize}
                \item \textbf{true}: You handled the event; stop it from going further
                \item \textbf{false}: Event continues to propagate to underlying views or the view itself 
            \end{itemize}
            Attach the listener to a view with
            \begin{javacode}
                v.setOnTouchListener(TouchHandler);
            \end{javacode}
        \item \textbf{GestureDetector.OnGestureListener}: An interface that defines six callback methods used to detect general gestures such as:
            \begin{itemize}
                \item Scroll
                \item Fling
                \item Long press
                \item Show press
                \item Single tap up
                \item Down event acknowledgment
            \end{itemize}
            ...
            Where
            \begin{itemize}
                \item \textbf{boolean onDown(MotionEvent e)}: Called immediately when the user first touches the screen (ACTION\_DOWN).
                    \bigbreak \noindent 
                    ...
                    \bigbreak \noindent 
                    \textbf{Note:} This is often the first method invoked for any gesture. If you return false, some gestures may not trigger follow-up callbacks.
                \item \textbf{void onShowPress(MotionEvent e)}: Called when the user has touched the screen and is holding still just long enough that the system believes a press is intentional, but before the long-press timeout is reached.
                    \bigbreak \noindent 
                    ...
                    \bigbreak \noindent 
                    \textbf{Note:} No return value. It does not mean the user will long-press.
                \item \textbf{boolean onSingleTapUp(MotionEvent e)}: Called when the user lifts their finger after a quick tap.
                    Simple tap recognition, UI element activation (if not using OnClickListener). Triggering lightweight actions
                    \bigbreak \noindent 
                    \textbf{Note:} Returning true indicates you handled the tap.
                \item \textbf{boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY)}: Called when the user drags/moves their finger across the screen slowly.
                    \bigbreak \noindent 
                    Parameters:
                    \begin{itemize}
                        \item \textbf{e1: the initial DOWN event}
                        \item \textbf{e2: the current MOVE event}
                        \item \textbf{distanceX: horizontal distance moved since last callback}
                        \item \textbf{distanceY: vertical distance moved since last callback}
                    \end{itemize}
                    \textbf{Note:} This method fires repeatedly during movement. It represents a scroll gesture, not a fling.
                    \bigbreak \noindent 
                ...
                \item \textbf{void onLongPress(MotionEvent e)}: Called when the user has pressed and held for the long-press timeout, typically $\sim$500 ms.
                    \bigbreak \noindent 
                    This is the exact moment Android recognizes a long-press gesture.
                \item \textbf{boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY)}: Called when the user quickly swipes and lifts their finger with momentum (fast movement + release).
                    \bigbreak \noindent 
                    Parameters:
                    \begin{itemize}
                        \item \textbf{e1: initial DOWN event}
                        \item \textbf{e2: final UP event}
                        \item \textbf{velocityX: horizontal swipe velocity (pixels/second)}
                        \item \textbf{velocityY: vertical swipe velocity (pixels/second)}
                    \end{itemize}
                    A fling is not just a scroll; it requires sufficient speed.
            \end{itemize}

        \item \textbf{GestureDetector.OnDoubleTapListener}: GestureDetector.OnDoubleTapListener is an interface that allows you to receive callbacks specifically related to double-tap gestures and related tap-state events. It is an optional companion to a GestureDetector.OnGestureListener. You attach it to a GestureDetector when you want to detect
            \begin{itemize}
                \item A confirmed double tap
                \item The separate events that happen before and after a double tap
                \item The two distinct tap actions inside that double tap
            \end{itemize}
            ...
            Where
            \begin{itemize}
                \item \textbf{boolean onSingleTapConfirmed(MotionEvent e)}: This is triggered when the system is sure the gesture was a single tap, and not part of a double tap. It fires after the double-tap timeout passes.
                    \bigbreak \noindent 
                ...
                \item \textbf{boolean onDoubleTap(MotionEvent e)}: This triggers when the user taps twice in quick succession according to Android’s double-tap detection timing. This method fires once for the entire double-tap gesture (i.e., when the second tap occurs). If you want to zoom in/out or run any “double-tap special action,” this is the correct place.
                \item \textbf{boolean onDoubleTapEvent(MotionEvent e)}: This method receives all events that occur during the double-tap gesture, including:
                    \begin{itemize}
                        \item ACTION\_DOWN of the second tap
                        \item ACTION\_MOVE events between the two taps
                        \item ACTION\_UP events
                        \item Small drags during a double-tap sequence
                    \end{itemize}
            ...
            \end{javacode}
            \bigbreak \noindent 
            Where \texttt{gestureListener} is a class that implements GestureDetector.OnGestureListener, and \texttt{doubleTapListener} is a class that implements GestureDetector.OnDoubleTapListener.
            \bigbreak \noindent 
    ...
    \begin{itemize}
        ...
        \item \textbf{boolean onTouchEvent(MotionEvent event)}: onTouchEvent is a built-in method of every View in Android. 
            \bigbreak \noindent 
            Its purpose is to:
            \begin{enumerate}
                \item Receive low-level touch events (DOWN, MOVE, UP) that reach the view.
                \item Decide whether the view will handle them or ignore them.
                \item Trigger higher-level behaviors such as clicks, long-presses, scrolling, focusing, etc.
            \end{enumerate}
            Every custom view and every standard widget (Button, TextView, ImageView, etc.) has its own implementation of this method.
            \bigbreak \noindent 
            The sequence is:
            \begin{enumerate}
                \item User touches screen $\to$ system creates a MotionEvent.
                \item Android routes the event to the view under the touch.
                \item If a View.OnTouchListener is attached:
                    \begin{itemize}
                        \item Android calls listener.onTouch(v, event) first.
                        \item If that method returns true, the event is consumed, and view.onTouchEvent() will NOT be called.
                    \end{itemize}
                \item If the listener returns false, then Android calls the view’s own onTouchEvent(event).
            \end{enumerate}
            The default implementation in View handles
            \begin{enumerate}
                \item \textbf{Click detection}: If the sequence of MotionEvents is DOWN, optional small MOVE, UP, then onTouchEvent interprets that as a click and calls
                    \bigbreak \noindent 
                    \begin{javacode}
                        performClick();
                    \end{javacode}
                ...
                \item \textbf{Long-press detection}: If the finger stays down long enough, it triggers:
                    \bigbreak \noindent 
                    \begin{javacode}
                        performLongClick();
                    \end{javacode}
                ...
                \item \textbf{Focusable and selectable states}
                \item \textbf{Scrolling for scrollable views}
            \end{enumerate}
            ...
            \begin{javacode}
                ...
                public boolean onTouchEvent(MotionEvent event) {
                    switch (event.getAction()) {
                        ...
                        // Start tracking
                        return true;   // Important: we claim the gesture
            ...
            \end{javacode}
            \bigbreak \noindent 
            We return true on ACTION\_DOWN because it tells android “This view is handling this gesture; send me all future MOVE and UP events.” If you return false on ACTION\_DOWN, Android stops sending you the rest of the gesture.
        \item \textbf{Why onTouchEvent Matters When Using GestureDetector}: When we do
            \bigbreak \noindent 
            \begin{javacode}
                @Override
                public boolean onTouchEvent(MotionEvent event) {
            ...
            \begin{itemize}
                \item “GestureDetector will decide what to do with this event.”
                \item “GestureDetector’s callbacks (onScroll, onFling, etc.) will replace default View behavior.”
            \end{itemize}
    ...
    \subsection{GridLayout}
    \begin{itemize}
        \item \textbf{Setting orientation}: n GridLayout (Java), you set the orientation directly on the GridLayout instance. The orientation determines how automatic placement proceeds—either left-to-right then top-to-bottom (horizontal), or top-to-bottom then left-to-right (vertical).
            \bigbreak \noindent 
            The two valid constants are:
            \begin{itemize}
                \item GridLayout.HORIZONTAL
                \item GridLayout.VERTICAL
            \end{itemize}
        \item \textbf{Positioning}:  In GridLayout, every child view lives in a “cell” identified by a (row, column) pair, with indices starting at 0. Positioning is controlled by GridLayout.LayoutParams, specifically its rowSpec and columnSpec fields.
            \bigbreak \noindent 
            There are two ways in which we can position items in a GridLayout
            \begin{enumerate}
                \item \textbf{Default}: Let GridLayout place in the next available position. The Grid 
                \item \textbf{Specified}: We can specify an exact (row, col) position to place views at.
            \end{enumerate}
        \item \textbf{The GridLayout.Spec object}: A GridLayout.Spec is an object that describes how a single child occupies the grid—specifically, which rows or columns it occupies, how many cells it spans, and what its alignment is within those cells.
            \bigbreak \noindent 
            A child has two specs:
            \begin{enumerate}
                \item \textbf{rowSpec}: Defines row index and row span
                \item \textbf{columnSpec}: Defines column index and column span
            \end{enumerate}
            ...
            \begin{enumerate}
                \item \textbf{Start index}: The row or column where the child begins
                \item \textbf{Span}: How many rows or columns it covers
                \item \textbf{Alignment (optional)}: How the child aligns inside its cells
            \end{enumerate}
            \bigbreak \noindent 
            We create a GridLayout.Spec object by calling GridLayout.spec(...)
        \item \textbf{Default positioning}: 
            \bigbreak \noindent 
            \begin{javacode}
                // rowSpec and columnSpec say where this view goes
            ...
            \end{javacode}
            \bigbreak \noindent 
            \texttt{GridLayout.UNDEFINED} Means “I am not specifying this index; let GridLayout figure it out.”
        \item \textbf{Automatic placement with spanning}: Now suppose you want a view that spans multiple cells but you still do not care where it lands, only that a block of that size fits.
            \bigbreak \noindent 
            Example: automatically place a view that is 2 columns wide and 1 row high.
            \bigbreak \noindent 
            \begin{javacode}
                // row index: unspecified, 1 row high
                GridLayout.Spec rowSpec = GridLayout.spec(GridLayout.UNDEFINED, 1);

                // column index: unspecified, span 2 columns
                GridLayout.Spec colSpec = GridLayout.spec(GridLayout.UNDEFINED, 2);
            \end{javacode}
        \item \textbf{Explicit positioning (specific row and column)}: To place a view in a specific cell (row, column):
            \bigbreak \noindent 
            \begin{javacode}
                // Place at row 1, column 2 (second row, third column)
                GridLayout.Spec rowSpec = GridLayout.spec(1); // row index 1
                GridLayout.Spec colSpec = GridLayout.spec(2); // column index 2

                GridLayout.LayoutParams params = new GridLayout.LayoutParams(rowSpec, colSpec);
                grid.addView(button, params);
            \end{javacode}
        \item \textbf{(android.widget.GridLayout) Getting the position of a view in a GridLayout}: The API does not expose publicly which position a view is placed in, so we must track this information ourselves
        \item \textbf{(androidx.gridlayout.widget.GridLayout) Getting the position of a view in a GridLayout}: You obtain the (row, column) pair of a child view by reading its GridLayout.LayoutParams and then inspecting the rowSpec and columnSpec inside those layout parameters.
            \bigbreak \noindent 
            ...
            \begin{itemize}
                \item \textbf{Starting row / column indices}: 
                    \bigbreak \noindent 
                    \begin{javacode}
                        int rowSpan = rowSpec.getSpan().size();
                        int colSpan = colSpec.getSpan().size();
                    \end{javacode}
                \item \textbf{Ending row / column indices}: 
                    \bigbreak \noindent 
                    \begin{javacode}
                        int rowEnd = rowSpec.getSpan().max;   // inclusive
                        int colEnd = colSpec.getSpan().max;   // inclusive
                    \end{javacode}
                \item \textbf{Span length}: 
                    \bigbreak \noindent 
                    \begin{javacode}
                        int rowSpan = rowSpec.getSpan().size();
                        int colSpan = colSpec.getSpan().size();
                ...
                \item \textbf{Alignment}:
                    \bigbreak \noindent 
                    \begin{javacode}
                        GridLayout.Alignment align = rowSpan.getAlignment();
                    \end{javacode}
            \end{itemize}
        \item \textbf{Getting child at (row, column) position}: here is no built-in API that maps (row, column) $\to$ child view, we must track this information ourselves.
